...
async function tffOfficialFixturesForDate(date){
  const k=`tff-official:${date}`,c=getCache(k);if(c)return c;
  const out=[];
  const pages=[
  ...
  for(const p of pages){
    try{
      const raw=await fetchTextSmart(p.url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.10.7","Accept-Language":"tr-TR,tr;q=0.9"});
      const text=decodeHtmlText(raw);
      const dd=date.slice(8,10)+"."+date.slice(5,7)+"."+date.slice(0,4);
      // TFF rows render as: DD.MM.YYYY HH:MM HOME [score or -] AWAY Detaylar
      const re=new RegExp(dd.replace(/\./g,"\\.")+String.raw`\s+(\d{1,2}:\d{2})\s+([^\n]{2,90}?)\s+(?:\d+\s*-\s*\d+|-\s*)\s*([^\n]{2,90}?)\s+(?:Detaylar|$)`,"gi");
      let m;
      while((m=re.exec(text))){
        const clean=x=>String(x).replace(/\s+/g," ").replace(/\s+(A\.Ş\.|FK|SK)$/i,mm=>mm).trim();
        const f=officialFixture(p.code,date,m[1],clean(m[2]),clean(m[3]),"tff-official");
        if(f)out.push(f);
      }
      // Alternate text layout sometimes keeps the separator on its own line.
      if(!out.some(x=>x.leagueCode===p.code)){
        const lines=text.split("\n").map(x=>x.trim()).filter(Boolean);
        for(let i=0;i<lines.length;i++){
          const mm=lines[i].match(new RegExp(`^${dd.replace(/\./g,"\\.")}\\s+(\\d{1,2}:\\d{2})\\s+(.+?)\\s+-\\s+(.+?)(?:\\s+Detaylar)?$`,"i"));
          if(mm){
            const f=officialFixture(p.code,date,mm[1],mm[2].trim(),mm[3].replace(/\s+Detaylar.*$/i,"").trim(),"tff-official");
            if(f)out.push(f);
          }
        }
      }
...
async function tffLeagueHistory(code,date){
  ...
  const k=`tff-history:${code}:${date}`,c=getCache(k);if(c)return c;
  const url=code==="T1L"?"https://www.tff.org/default.aspx?pageID=142":"https://www.tff.org/default.aspx?pageID=198";
  try{
    const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.10.7","Accept-Language":"tr-TR,tr;q=0.9"});
    const text=decodeHtmlText(raw),out=[];
    for(const line of text.split("\n").map(x=>x.replace(/\s+/g," ").trim()).filter(Boolean)){
      const m=line.match(/^(\d{2}\.\d{2}\.\d{4})\s+(\d{1,2}:\d{2})\s+(.+?)\s+(\d+)\s*-\s*(\d+)\s+(.+?)(?:\s+Detaylar)?$/i);
      if(!m)continue;
      const ymd=trDateToYmd(m[1]); if(!ymd||ymd>=date)continue;
      out.push({season:"2026/27",date:new Date(`${ymd}T12:00:00+03:00`),
        home:tffCleanTeamName(m[3]),away:tffCleanTeamName(m[6]),homeGoals:Number(m[4]),awayGoals:Number(m[5]),
    ...
    const ded=[...new Map(out.map(m=>[`${m.date.toISOString()}|${norm(m.home)}|${norm(m.away)}`,m])).values()].sort((a,b)=>a.date-b.date);
    setCache(k,ded,300000); return ded;
  }catch(e){console.warn("TFF history:",e.message);return[]}
}

async function swissOfficialFixturesForDate(date){
  const k=`sfl-official:${date}`,c=getCache(k);if(c)return c;
  const out=[];
  try{
    // Official Swiss Football League Match Center, current Super League season.
    const seasonEnd=Number(date.slice(0,4))+1;
    const url=`https://matchcenter-sfl.football.ch/Default.aspx?a=mag&lng=1&ls=25694&oid=2&s=${seasonEnd}&sg=70075`;
    const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.10.7","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
      const text=decodeHtmlText(await r.text());
      const dd=date.slice(8,10)+"."+date.slice(5,7)+"."+date.slice(0,4);
      const lines=text.split("\n").map(x=>x.replace(/\s+/g," ").trim()).filter(Boolean);
      for(let i=0;i<lines.length;i++){
...
async function swissChallengeOfficialFixturesForDate(date){
  const k=`sfl-challenge-official:${date}`,c=getCache(k);if(c)return c;
  const out=[];
  try{
    const seasonEnd=Number(date.slice(0,4))+1;
    ...
    if(r.ok){
      const text=decodeHtmlText(await r.text()),dd=date.slice(8,10)+"."+date.slice(5,7)+"."+date.slice(0,4);
      const lines=text.split("\n").map(x=>x.replace(/\s+/g," ").trim()).filter(Boolean);
      for(let i=0;i<lines.length;i++){
...
async function officialFixturesForDate(date){
  const [tr,ch,ch2]=await Promise.all([tffOfficialFixturesForDate(date),swissOfficialFixturesForDate(date),swissChallengeOfficialFixturesForDate(date)]);
  return [...tr,...ch,...ch2];
}
...
function mapFixture(x){
  ...
  return {id:x.fixture.id,date:x.fixture.date,localDate:localYmdFromDate(x.fixture.date),displayTime:localTimeFromDate(x.fixture.date),timestamp:x.fixture.timestamp,status:x.fixture.status?.short||"",leagueCode:l.code,league:l.name,country:l.country,emoji:l.emoji,round:x.league.round||"",home:{id:x.teams.home.id||null,name:x.teams.home.name,logo:x.teams.home.logo||""},away:{id:x.teams.away.id||null,name:x.teams.away.name,logo:x.teams.away.logo||""},
...
async function fixturesForThreeDays(centerDate){
  const k=`three-days-v7107:${centerDate}`,cached=getCache(k); if(cached) return cached;
  const dates=[shiftYmd(centerDate,-1),centerDate,shiftYmd(centerDate,1)];
  let apiRows=[],fdRows=[],espnRows=[],officialRows=[];
  ...
  let merged=mergeFixtures(apiRows,[...fdRows,...espnRows,...officialRows]).filter(f=>dates.includes(f.localDate));
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.10.7-tff-history-charset",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
async function analyzeFixtureModel(f,date){
  ...
  if(!allLeague.length&&["TSL","T1L"].includes(f.leagueCode)){
  ...
  model.researchSource=allLeague.length?"league-history":(f.espnSlug?"espn-team-history":"api-football-league-history");
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.10.7 running on port ${PORT}`));
