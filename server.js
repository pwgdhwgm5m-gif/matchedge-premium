...
async function loadCSV(season,div){
  ...
  try{
    for(let attempt=0;attempt<2;attempt++){
      try{
        const r=await fetch(fdUrl(season,div),{headers:{"User-Agent":"MatchEdge/7.18.0","Accept":"text/csv,text/plain,*/*"}});
...
async function loadNewLeagueCSV(code){
  ...
  try{
    const r=await fetch(`https://www.football-data.co.uk/new/${file}.csv`,{
      headers:{"User-Agent":"MatchEdge/7.18.0","Accept":"text/csv,text/plain,*/*"}
    });
...
async function leagueHistory(code){
  ...
  if(l.csv){
    ...
    all=rows.map(completedNew).filter(Boolean)
      .filter(m=>{
        if(code==="RPL")return m.season==="2025/26";
...
function mergeOddsSources(apiOdds,fdOdds,fixtureId){
  if(!apiOdds&&!fdOdds)return null;
  const current={...(fdOdds?.closing||{}),...(apiOdds?.odds||{})};
  ...
  for(const [name,o] of Object.entries(opening)){
    ...
    movement[name]={
      ...
      firstSeen:"football-data-opening",lastSeen:apiOdds?.providerUpdatedAt||apiOdds?.fetchedAt||fdOdds?.fetchedAt||null,
      openingProvider:"Football-Data.co.uk"
    };
  }
  return {
    odds:current,opening,bookmakers,movement,
    provider:apiOdds?.provider||fdOdds?.provider||null,
    providers:[...new Set([apiOdds?.provider,fdOdds?.provider].filter(Boolean))],
    fetchedAt:apiOdds?.fetchedAt||fdOdds?.fetchedAt||new Date().toISOString(),
    providerUpdatedAt:apiOdds?.providerUpdatedAt||null
  };
}
...
function selectBetRecommendation(valueMarkets=[]){
  ...
  if(withOdds.length){
    const ranked=withOdds.filter(x=>x.probability>=54&&x.edge>=3&&x.expectedValue>=4&&x.confidence>=45)
      .map(x=>{
        const move=Number(x.impliedMovePts);
        let movementScore=0;
        if(Number.isFinite(move)){
          // Shortening (positive implied move) confirms a selection; drifting is a warning.
          movementScore=clamp(move,-6,6);
        }
        return {...x,movementScore,valueScore:x.expectedValue+x.edge*.6+x.confidence*.12+movementScore*.9};
      })
      .sort((a,b)=>b.valueScore-a.valueScore);
    if(ranked[0]){
      const top=ranked[0];
      if(Number(top.impliedMovePts)<=-5&&top.edge<6)return {decision:"NO BET",reason:"market-drift-conflict"};
      return {...top,decision:"BET",reason:Number.isFinite(Number(top.impliedMovePts))?"positive-value-with-market-context":"positive-value"};
    }
    return {decision:"NO BET",reason:"no-positive-value"};
  }
  const ranked=valueMarkets.filter(x=>x.probability>=60&&x.confidence>=50).sort((a,b)=>b.probability-a.probability);
...
function marketAwareMarkets(valueMarkets=[],dataQuality=60){
  ...
  return (valueMarkets||[]).map(x=>{
    ...
    // Market is context, not a replacement for the statistical model.
    // Stronger data -> slightly less market weight; weak/early samples get a little more.
...
function buildModel(h,home,away,leagueH=h,fixtureOdds=null){
  ...
  const analysisMarkets=marketAwareMarkets(valueMarkets,dq);
  ...
  const noBet=betRecommendation?.decision==="BET"
    ? false
    : (!strongest||strongestP<57||strongestC<45);
...
async function sportsDbFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"MatchEdge/7.18.0"}});
    if(!r.ok)return[];
    const b=await r.json(),out=[];
    for(const ev of b?.events||[]){
...
async function tffOfficialFixturesForDate(date){
  ...
  for(const p of pages){
    try{
      const raw=await fetchTextSmart(p.url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.0","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function tffLeagueHistory(code,date){
  ...
  try{
    const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.0","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function swissOfficialFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.0","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
  ...
  if(!out.length){
    try{
      const raw=await fetchTextSmart("https://matchcenter-sfl.football.ch/Default.aspx?a=sp&lng=1&ls=25694&oid=2&sg=70075",{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.0"});
...
async function svffOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.svenskfotboll.se/serier-cuper/elitfotboll",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.18.0","Accept-Language":"sv-SE,sv;q=0.9,en;q=0.7"
    });
    const text=decodeHtmlText(raw);
...
async function russianCupOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.rfs.ru/cup/tournament/matches/rpl",{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.0","Accept-Language":"ru-RU,ru;q=0.9"});
...
async function polishCupFixturesForDate(date){
  ...
  const out=[],p=datePartsForLocale(date);
  try{
    const raw=await fetchTextSmart("https://www.legalsport.pl/rozgrywki/pilka-nozna/puchar-polski/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.0","Accept-Language":"pl-PL,pl;q=0.9"});
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function fixturesForThreeDays(centerDate){
  const k=`daily-v7180:${centerDate}`,cached=getCache(k); if(cached) return cached;
  const dates=[centerDate];
  const [denDirect,den2Direct,rplDirect,ruscDirect]=await Promise.all([
  ...
  let merged=mergeFixtures(apiRows,[...fdRows,...espnRows,...officialRows,...sportsDbRows,...romaniaRows,...denDirect,...den2Direct,...rplDirect,...ruscDirect]).filter(f=>dates.includes(f.localDate));
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.18.0-market-aware-analysis",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:false,timezoneNormalization:true,fixtureMerge:true,romaniaStructuredOnly:true,analysisToggle:true,marketAwareAnalysis:true,noVigMarketProbability:true}}));
...
async function romanianLiveScoresForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.gsp.ro/rezultate-live/",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.18.0","Accept-Language":"ro-RO,ro;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function austriaBundesligaHistory(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.bundesliga.at/de/spielplan",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.18.0","Accept-Language":"de-AT,de;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
    const out=[];
    ...
    if(out.length<6){
      try{
        const fb=await fetchTextSmart("https://fbref.com/en/comps/56/2026-2027/schedule/2026-2027-Austrian-Bundesliga-Scores-and-Fixtures",{
          "User-Agent":"Mozilla/5.0 MatchEdge/7.18.0"
        });
...
async function polandEkstraklasaHistory(date){
  ...
  for(const url of urls){
    try{
      const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.0","Accept-Language":"pl-PL,pl;q=0.9,en;q=0.7"});
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function analyzeFixtureModel(f,date){
  ...
  let liveOdds=null;
  const [apiOdds,fdOdds]=await Promise.all([
    f.apiFixtureId?apiFixtureMarketOdds(f):Promise.resolve(null),
    footballDataFixtureOdds(f,date)
  ]);
  liveOdds=mergeOddsSources(apiOdds,fdOdds,f.apiFixtureId);
  const model=buildModel(leagueH,home,away,leagueH,liveOdds);
  ...
  setCache(modelKey,model,liveOdds?120000:300000);
...
app.get("/api/debug/odds/:id",async(req,res)=>{
  try{
    ...
    const [apiOdds,fdOdds]=await Promise.all([f.apiFixtureId?apiFixtureMarketOdds(f):null,footballDataFixtureOdds(f,date)]);
    const merged=mergeOddsSources(apiOdds,fdOdds,f.apiFixtureId);
    res.json({ok:true,fixture:{id:f.id,apiFixtureId:f.apiFixtureId,leagueCode:f.leagueCode,home:f.home?.name,away:f.away?.name},apiOdds,footballData:fdOdds,merged});
  }catch(e){res.status(500).json({ok:false,error:e.message})}
});
...
function strongestSignalCard(m){
  ...
  const signal=x
    ? '<span>'+t("strongSignal")+'</span><strong>'+esc(translateMarketName(x.name))+(x.decision==="BET"?' · BET':'')+'</strong><small class="signalProb">'+t("signalProbability")+': '+(x.analysisProbability??x.probability)+'%'+(x.marketProbability!=null?' · Piyasa '+x.marketProbability+'%':'')+'</small>'
  ...
  const valueHtml=br.decision==="BET"
    ? '<div class="market"><b>'+t("betDecision")+' · '+esc(translateMarketName(br.name))+'</b><span class="prob">BET</span></div>'+
      ...
      '</div>'+(br.bookmaker?'<small>'+esc(br.bookmaker)+'</small>':'')
  ...
  const marketToggle='<details class="toggleBox signalMarketToggle"><summary>'+t("marketOddsAnalysis")+'</summary><div class="toggleBody">'+
    valueHtml+'<div class="section">'+t("strong")+'</div>'+marketRows(m.recommendations||[])+
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.18.0 running on port ${PORT}`));
