...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.10.3-league-fixture-fallback",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
async function analyzeFixtureModel(f,date){
  ...
  if(!home||!away||!h.length)throw new Error("Lig ve takım geçmiş verisi bulunamadı");
  const model=buildModel(h,home,away,leagueH.length?leagueH:h);
  model.researchSource=allLeague.length?"league-history":(leagueH.length?"api-football-league-history":"api-football-team-history");
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.10.3 running on port ${PORT}`));
