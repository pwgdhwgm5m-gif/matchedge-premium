...
async function footballDataUpcoming(date){
  ...
  try{
    ...
    for(const row of rows){
      ...
      const f={id,date:dt,localDate:date,displayTime:tm,timestamp:Math.floor(new Date(dt).getTime()/1000),status:"NS",leagueCode:l.code,league:l.name,country:l.country,emoji:l.emoji,round:"",home:{name:home,logo:""},away:{name:away,logo:""},
        score:{home:row.FTHG!==""&&row.FTHG!=null?Number(row.FTHG):null,away:row.FTAG!==""&&row.FTAG!=null?Number(row.FTAG):null,htHome:row.HTHG!==""&&row.HTHG!=null?Number(row.HTHG):null,htAway:row.HTAG!==""&&row.HTAG!=null?Number(row.HTAG):null},
    ...
    try{
      ...
      for(const ev of b.events||[]){
        ...
        const f={id,date:dt,localDate,displayTime:localTimeFromDate(dt),timestamp:Math.floor(new Date(dt).getTime()/1000),
          ...
          score:{home:hc.score!==undefined&&hc.score!==null&&hc.score!==""?Number(hc.score):null,away:ac.score!==undefined&&ac.score!==null&&ac.score!==""?Number(ac.score):null,htHome:null,htAway:null},
          elapsed:ev.status?.displayClock||comp.status?.displayClock||null,fixtureSource:"espn-fallback"};
        out.push(f); setCache(`fixture:${id}`,f,21600000);
      }
      return out;
    }catch{return[];}
...
function mergeFixtures(apiRows,fdRows){
  ...
  for(const f of apiRows||[]){
    ...
    map.set(k,old?{...old,...f,home:{...old.home,...f.home},away:{...old.away,...f.away},score:{...(old.score||{}),...(f.score||{})},fixtureSource:"merged"}:{...f,fixtureSource:"api-football"});
  }
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.8.1-live-scores",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
app.get("/api/live",async(req,res)=>{
  try{
    ...
    const fixtures=(body.response||[]).map(mapFixture).filter(Boolean);
...
function riskTierForPick(model,pick){
  const p=Number(pick?.probability||0),c=Number(pick?.confidence||0),q=Number(model?.dataQuality||0);
  if(p>=72&&c>=62&&q>=68)return"low";
  if(p>=64&&c>=52&&q>=58)return"medium";
  if(p>=57&&c>=44&&q>=52)return"high";
  return null;
}
function dayPickScore(x){
  const pref={["Çifte Şans"]:12,["Gol"]:11,["1X2"]:9,["KG"]:8,["Takım Gol"]:7,["İlk Yarı"]:3,["İkinci Yarı"]:0,["Yarı"]:0,["Korner"]:-20};
  return x.probability*.58+x.confidence*.32+(pref[x.group]||0);
}
function modelFairOdd(probability){
  const p=Math.max(.01,Math.min(.99,Number(probability||0)/100));
  return +(1/p).toFixed(2);
}
function chooseDayOptions(model){
  // Daily coupon engine only uses mainstream markets that are normally offered
  // by bookmakers. Corner lines stay visible in match analysis, but are excluded
  // from coupon construction unless a real current price feed is added later.
  const allowedGroups=["Çifte Şans","Gol","1X2","KG","Takım Gol","İlk Yarı"];
  const xs=(model.markets||[])
    .filter(x=>allowedGroups.includes(x.group))
    .filter(x=>x.probability>=57&&x.confidence>=44)
    .filter(x=>!(x.group==="İlk Yarı"&&x.probability<66))
    .sort((a,b)=>dayPickScore(b)-dayPickScore(a));
  const out=[];const groups=new Set();
  for(const x of xs){
    if(groups.has(x.group))continue;
    out.push({...x,fairOdd:modelFairOdd(x.probability)});groups.add(x.group);
    if(out.length>=2)break;
  }
  return out;
}

app.get("/api/day-picks",async(req,res)=>{
  try{
    const date=String(req.query.date||"");
    if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return res.status(400).json({ok:false,error:"Geçerli tarih gerekli."});
    const cacheKey=`day-picks-v772:${date}`,cached=getCache(cacheKey);if(cached)return res.json(cached);

    const fixtures=(await fixturesForDate(date)).filter(f=>!statusFinishedServer(f.status));
    // Load Football-Data histories once, not once per fixture.
    const csvCodes=Object.keys(LEAGUES).filter(c=>LEAGUES[c].csv);
    await Promise.all(csvCodes.map(leagueHistory));

    const results=[];let idx=0;
    const workers=Array.from({length:Math.min(8,fixtures.length||1)},async()=>{
      while(true){
        const i=idx++;if(i>=fixtures.length)break;
        const f=fixtures[i];
        try{
          const model=await analyzeFixtureModel(f,date);
          if(model.noBet)continue;
          const options=chooseDayOptions(model);
          for(const pick of options){
            const risk=riskTierForPick(model,pick);if(!risk)continue;
            results.push({
              fixtureId:f.id,leagueCode:f.leagueCode,league:f.league,country:f.country,emoji:f.emoji,
              home:f.home.name,away:f.away.name,time:f.displayTime,
              pick:pick.name,group:pick.group,probability:pick.probability,confidence:pick.confidence,
              fairOdd:pick.fairOdd||modelFairOdd(pick.probability),
              dataQuality:model.dataQuality,risk,expectedGoals:model.expectedGoals,likelyScore:model.likelyScore
            });
          }
        }catch{}
      }
    ...
    results.sort((a,b)=>(riskRank[b.risk]-riskRank[a.risk])||(dayPickScore(b)-dayPickScore(a)));

    // Build coupons by combined FAIR odds, while risk is still based on
    // model probability/confidence/data quality. High combined odds do not
    // automatically mean every leg is high-risk.
    function combinedFairOdd(arr){
      return +arr.reduce((o,x)=>o*Number(x.fairOdd||modelFairOdd(x.probability)),1).toFixed(2);
    }
    function buildCoupon(profile){
      const cfg={
        low:{allowed:["low"],min:1.45,max:2.25,minLegs:2,maxLegs:3},
        medium:{allowed:["low","medium"],min:2.40,max:4.80,minLegs:3,maxLegs:5},
        high:{allowed:["medium","high","low"],min:4.50,max:9.50,minLegs:4,maxLegs:6}
      }[profile];
      const pool=results
        .filter(x=>cfg.allowed.includes(x.risk))
        .sort((a,b)=>dayPickScore(b)-dayPickScore(a));
      const out=[];
      for(const x of pool){
        if(out.some(y=>y.fixtureId===x.fixtureId))continue;
        const candidate=[...out,x];
        const odd=combinedFairOdd(candidate);
        // Avoid inflating coupon with unnecessary legs once target range is reached.
        if(out.length>=cfg.minLegs && combinedFairOdd(out)>=cfg.min)break;
        if(odd>cfg.max && out.length>=cfg.minLegs)continue;
        out.push(x);
        if(out.length>=cfg.maxLegs)break;
      }
      // If target was not reached, add best remaining unique fixtures until max legs.
      if(combinedFairOdd(out)<cfg.min){
        for(const x of pool){
          if(out.length>=cfg.maxLegs)break;
          if(out.some(y=>y.fixtureId===x.fixtureId))continue;
          out.push(x);
      ...
      return {legs:out,fairCombinedOdd:combinedFairOdd(out)};
    }
    const coupons={
      low:buildCoupon("low"),
      medium:buildCoupon("medium"),
      high:buildCoupon("high")
    };
    const payload={ok:true,date,count:results.length,picks:results,coupons,analyzed:fixtures.length};
...
.dailyPicks{margin:8px 0 18px;border:1px solid #d9b56f38;border-radius:17px;overflow:hidden;background:#d9b56f09}
...
.dailyPicksBody{padding:0 12px 12px}.riskGroup{margin-top:10px}.riskTitle{font-size:10px;font-weight:900;letter-spacing:.3px;margin-bottom:6px}
.pickCard{padding:10px 11px;margin:6px 0;border:1px solid #ffffff12;border-radius:12px;background:#ffffff06}
.pickCard b{display:block;font-size:11px}.pickCard span{font-size:9px;color:#9ba8b8}.pickMeta{display:flex;justify-content:space-between;margin-top:6px;font-size:9px}
.couponBox{margin-top:12px;padding:11px;border:1px dashed #d9b56f55;border-radius:12px}.couponBox b{font-size:11px;color:#d9b56f}.couponLeg{font-size:10px;padding:5px 0;border-bottom:1px solid #ffffff0d}
...
function applyStatic(){
 ...
 sumMatches.textContent=t("selected");sumAnalysis.textContent=t("open");threeDayStrong.textContent=t("three");sumDays.textContent=t("dayLine");footerNote.textContent=t("footer");dailyPicksTitle.textContent=t("dailyTitle");
 document.documentElement.lang=lang
}
function setLang(v){lang=v;localStorage.setItem("matchedge_lang",v);closeOpenAnalysis();applyStatic();chips();renderDays();render();dailyPicksBody.innerHTML='<div class="loader">'+t("loadingPicks")+'</div>';dailyPicks.dataset.loaded="";}
function renderDays(){const defs=[{label:t("y"),date:new Date(shiftYmdClient(SERVER_TODAY,-1)+"T12:00:00Z")},{label:t("t"),date:new Date(SERVER_TODAY+"T12:00:00Z")},{label:t("tm"),date:new Date(shiftYmdClient(SERVER_TODAY,1)+"T12:00:00Z")}];days.innerHTML=defs.map(x=>{const id=iso(x.date);return'<button class="day '+(id===selectedDate?'active':'')+'" data-date="'+id+'"><b>'+x.label+'</b><span>'+x.date.getUTCDate()+'</span><small>'+esc(x.date.toLocaleDateString(locale(),{month:"short",timeZone:"UTC"}))+'</small></button>'}).join("");days.querySelectorAll("button").forEach(b=>b.onclick=()=>{selectedDate=b.dataset.date;closeOpenAnalysis();dailyPicks.dataset.loaded="";dailyPicksBody.innerHTML='<div class="loader">'+t("loadingPicks")+'</div>';renderDays();render();});}
...
function applyLiveFixtures(live){
  ...
  for(const lf of live){
    ...
    if(idx>=0){allFixtures[idx]={...allFixtures[idx],...lf,home:{...allFixtures[idx].home,...lf.home},away:{...allFixtures[idx].away,...lf.away},score:{...(allFixtures[idx].score||{}),...(lf.score||{})}};changed=true;}
  }
  if(changed)render();
}
...
function render(){
  const a=filtered();mc.textContent=a.length;fc.textContent=a.length+" "+t("fixture");
...
let dailyPicksSeq=0;
function translateRisk(r){return r==="low"?t("lowRisk"):r==="medium"?t("mediumRisk"):t("highRisk")}
function renderDailyPicks(data){
  const groups={low:[],medium:[],high:[]};(data.picks||[]).forEach(x=>(groups[x.risk]||groups.high).push(x));
  const card=x=>'<div class="pickCard"><b>'+esc(x.home)+' – '+esc(x.away)+' · '+esc(translateMarketName(x.pick))+'</b><span>'+esc(countryName(x.country))+' · '+esc(x.league)+' · '+esc(x.time||"")+'</span><div class="pickMeta"><span>'+t("prob")+': '+x.probability+'%</span><span>'+t("conf")+': '+x.confidence+'%</span><span>'+t("fairOdd")+': '+Number(x.fairOdd||0).toFixed(2)+'</span></div><div style="font-size:9px;color:#8796a9;margin-top:6px">'+t("modelView")+': xG '+x.expectedGoals.home+'–'+x.expectedGoals.away+' · '+x.likelyScore+' · '+t("qualityShort")+': '+x.dataQuality+'/100</div></div>';
  let html="";
  for(const r of ["low","medium","high"]){if(groups[r].length)html+='<div class="riskGroup"><div class="riskTitle">'+translateRisk(r)+'</div>'+groups[r].map(card).join("")+'</div>';}
  if(!html)html='<div class="empty">'+t("noPicks")+'</div>';
  const cs=data.coupons||{};
  const couponBlock=(title,obj)=>{
    const arr=obj?.legs||[];
    if(!Array.isArray(arr)||arr.length<2)return"";
    return'<div class="couponBox"><b>'+title+'</b><div style="font-size:9px;color:#8796a9;margin-top:4px">'+t("combinedOdd")+': '+Number(obj.fairCombinedOdd||0).toFixed(2)+'</div>'+arr.map((x,i)=>'<div class="couponLeg">'+(i+1)+'. '+esc(x.home)+' – '+esc(x.away)+' · '+esc(translateMarketName(x.pick))+' <span>('+x.probability+'% · '+Number(x.fairOdd||0).toFixed(2)+')</span></div>').join("")+'</div>';
  };
  html+=couponBlock(t("lowCoupon"),cs.low)+couponBlock(t("mediumCoupon"),cs.medium)+couponBlock(t("highCoupon"),cs.high);
  dailyPicksBody.innerHTML=html;
}
async function loadDailyPicks(){
  if(dailyPicks.dataset.loaded===selectedDate)return;
  const seq=++dailyPicksSeq;dailyPicksBody.innerHTML='<div class="loader">'+t("loadingPicks")+'</div>';
  try{
    const r=await fetch("/api/day-picks?date="+encodeURIComponent(selectedDate)),j=await r.json();
    if(!j.ok)throw Error(j.error||"No data");if(seq!==dailyPicksSeq)return;
    dailyPicks.dataset.loaded=selectedDate;renderDailyPicks(j);
  }catch(e){if(seq!==dailyPicksSeq)return;dailyPicksBody.innerHTML='<div class="error">'+esc(e.message||"No data")+'</div>';}
}
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.8.1 running on port ${PORT}`));
