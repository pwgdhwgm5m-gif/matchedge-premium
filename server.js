...
async function loadCSV(season,div){
  ...
  try{
    for(let attempt=0;attempt<2;attempt++){
      try{
        const r=await fetch(fdUrl(season,div),{headers:{"User-Agent":"MatchEdge/7.19.0","Accept":"text/csv,text/plain,*/*"}});
...
async function ensureOddsApiSportKeys(){
  ...
  oddsApiSportMapPromise=(async()=>{
    try{
      const r=await fetch(`${ODDS_API_BASE}/sports/?apiKey=${encodeURIComponent(ODDS_API_KEY)}&all=true`);
...
async function sportsDbFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"MatchEdge/7.19.0"}});
    if(!r.ok)return[];
    const b=await r.json(),out=[];
    for(const ev of b?.events||[]){
...
async function swissOfficialFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.19.0","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
...
/* ---------------- Match detail: corners, cards, penalties, VAR, shots, saves, scorers ----------------
   Only available for fixtures actually sourced from API-Football (numeric fixture id),
   since ESPN/official/CSV sources do not expose this level of in-match detail.
   Throw-ins are NOT available from any connected provider; that field always stays null
   rather than being fabricated. ----------------------------------------------------- */
function statValue(stats,type){
  const row=(stats||[]).find(s=>String(s.type||"").toLowerCase()===type.toLowerCase());
  if(!row||row.value==null)return null;
  const v=row.value;
  if(typeof v==="string")return Number(v.replace("%","").trim())||null;
  return Number(v);
}
async function fetchFixtureMatchDetail(apiFixtureId){
  if(!API_KEY||!apiFixtureId)return null;
  const k=`match-detail-v1:${apiFixtureId}`,c=getCache(k);if(c)return c;
  try{
    const [statsRows,eventRows]=await Promise.all([
      apiFootball(`/fixtures/statistics?fixture=${encodeURIComponent(apiFixtureId)}`,90000).catch(()=>[]),
      apiFootball(`/fixtures/events?fixture=${encodeURIComponent(apiFixtureId)}`,90000).catch(()=>[])
    ]);
    if(!statsRows?.length&&!eventRows?.length)return null;
    const homeStats=statsRows?.[0]?.statistics||[],awayStats=statsRows?.[1]?.statistics||[];
    const homeTeamId=statsRows?.[0]?.team?.id??null,awayTeamId=statsRows?.[1]?.team?.id??null;
    const pair=(type)=>({home:statValue(homeStats,type),away:statValue(awayStats,type)});
    const scorers=[],cardsDetail=[],varEvents=[],penalties=[];
    for(const ev of eventRows||[]){
      const side=ev.team?.id===homeTeamId?"home":ev.team?.id===awayTeamId?"away":null;
      const minute=ev.time?.elapsed!=null?`${ev.time.elapsed}${ev.time.extra?"+"+ev.time.extra:""}`:null;
      const type=String(ev.type||""),detail=String(ev.detail||"");
      if(/^goal$/i.test(type)){
        scorers.push({minute,side,player:ev.player?.name||null,assist:ev.assist?.name||null,detail,ownGoal:/own goal/i.test(detail)});
        if(/penalty/i.test(detail))penalties.push({minute,side,player:ev.player?.name||null,scored:!/missed/i.test(detail)});
      }else if(/^card$/i.test(type)){
        cardsDetail.push({minute,side,player:ev.player?.name||null,type:detail});
      }else if(/var/i.test(type)){
        varEvents.push({minute,side,detail});
      }
    }
    const result={
      corners:pair("Corner Kicks"),shotsOnTarget:pair("Shots on Goal"),shotsTotal:pair("Total Shots"),
      possession:pair("Ball Possession"),fouls:pair("Fouls"),offsides:pair("Offsides"),saves:pair("Goalkeeper Saves"),
      cards:{
        home:{yellow:statValue(homeStats,"Yellow Cards")||0,red:statValue(homeStats,"Red Cards")||0},
        away:{yellow:statValue(awayStats,"Yellow Cards")||0,red:statValue(awayStats,"Red Cards")||0}
      },
      scorers,cardsDetail,var:varEvents,penalties,
      throwIns:null,
      hasStats:!!statsRows?.length,hasEvents:!!eventRows?.length,
      fetchedAt:new Date().toISOString()
    };
    setCache(k,result,180000);
    return result;
  }catch(e){console.warn("Match detail fetch:",e.message);return null;}
}
...
app.get("/api/match-detail/:id",async(req,res)=>{
  try{
    const id=String(req.params.id||"");
    const cached=getCache(`fixture:${id}`);
    if(!cached)return res.status(404).json({ok:false,error:"Fixture not found"});
    const numericId=cached.apiFixtureId||(/^\d+$/.test(String(cached.id))&&cached.fixtureSource==="api-football"?cached.id:null);
    if(!numericId)return res.json({ok:true,available:false,reason:"no-api-fixture-id"});
    const detail=await fetchFixtureMatchDetail(numericId);
    if(!detail)return res.json({ok:true,available:false,reason:"no-data"});
    res.json({ok:true,available:true,detail});
  }catch(e){res.status(500).json({ok:false,error:e.message})}
});
...
.sh{text-align:right;color:#d7e1ec}.sa{text-align:left;color:#d7e1ec}.scoreNum{font-size:14px;font-weight:950;text-align:center;color:#fff}.scoreDetail{display:flex;gap:4px;justify-content:center;margin-top:3px;flex-wrap:wrap}.scorePill{font-size:7px;padding:2px 5px;border-radius:99px;background:#ffffff0c;color:#9fb0c2;font-weight:900}.scorePillFt{background:#19dab21c;color:var(--teal)}
.scoreRowClickable{cursor:pointer}.scoreRowClickable:active{background:#ffffff06}.scoreChevron{grid-column:1/-1;text-align:center;font-size:9px;color:#5f6f82;margin-top:-2px}.scoreRowOpen .scoreChevron{color:var(--teal)}
.matchDetailPanel{display:none;padding:10px 8px 14px;border-bottom:1px solid #ffffff08;background:#08131f}.matchDetailPanel.open{display:block}
.mdStatRow{display:grid;grid-template-columns:1fr auto 1fr;gap:8px;align-items:center;padding:5px 0;font-size:10px}.mdVal{font-weight:950;color:#fff}.mdVal:first-child{text-align:right}.mdVal:last-child{text-align:left}.mdLabel{text-align:center;color:#8e9aae;font-size:8px;font-weight:900}.mdUnavailable{opacity:.5}.mdUnavailable .mdLabel{text-align:center;grid-column:1/-1}
.mdSection{margin-top:10px;padding-top:8px;border-top:1px solid #ffffff0a;font-size:8px;font-weight:950;letter-spacing:.5px;color:#9fb4c4}.mdEvents{margin-top:4px}.mdEvent{font-size:10px;color:#c7d3de;padding:4px 0}.mdEvent small{color:#7d8ea0}.mdEmpty{padding:12px;text-align:center;color:#8190a2;font-size:9px}
...
function renderScoreboard(){
  ...
  scoreboardBody.innerHTML=groups.map(g=>{
    ...
    return '<div class="scoreLeague">'+head+'</div>'+g.arr.map(f=>{
      ...
      const detail=(live||finished)&&htKnown?'<div class="scoreDetail"><span class="scorePill">'+t("htShort")+' '+f.score.htHome+'-'+f.score.htAway+'</span><span class="scorePill scorePillFt">'+t("ftShort")+' '+(scoreKnown(f)?f.score.home+'-'+f.score.away:'—')+'</span></div>':"";
      const clickable=live||finished;
      const rowCls=clickable?"scoreRow scoreRowClickable":"scoreRow";
      const rowAttr=clickable?' onclick="toggleMatchDetail(\''+esc(String(f.id))+'\',this)" data-fid="'+esc(String(f.id))+'"':"";
      const row='<div class="'+rowCls+'"'+rowAttr+'><div>'+esc(f.displayTime||"")+'</div><div class="sh">'+esc(f.home.name)+'</div><div><div class="scoreNum">'+sc+'</div><div class="scoreStatus">'+st+'</div>'+detail+'</div><div class="sa">'+esc(f.away.name)+'</div>'+(clickable?'<div class="scoreChevron">⌄</div>':'')+'</div>';
      const panel=clickable?'<div class="matchDetailPanel" id="mdp-'+esc(String(f.id))+'"></div>':'';
      return row+panel;
    }).join("");
  }).join("");
}
function matchDetailStatRow(label,home,away,suffix){
  if(home==null&&away==null)return '';
  return '<div class="mdStatRow"><span class="mdVal">'+(home==null?"—":home+(suffix||""))+'</span><span class="mdLabel">'+esc(label)+'</span><span class="mdVal">'+(away==null?"—":away+(suffix||""))+'</span></div>';
}
function matchDetailEventList(title,items,renderer){
  if(!items||!items.length)return '';
  return '<div class="mdSection">'+esc(title)+'</div><div class="mdEvents">'+items.map(renderer).join("")+'</div>';
}
function renderMatchDetail(id,homeName,awayName,payload){
  const el=document.getElementById("mdp-"+id);
  if(!el)return;
  if(!payload||payload.available===false){
    el.innerHTML='<div class="mdEmpty">'+t("noDetailData")+'</div>';
    el.classList.add("open");
    return;
  }
  const d=payload.detail;
  const sideName=(s)=>s==="home"?homeName:s==="away"?awayName:"—";
  let html='';
  html+=matchDetailStatRow(t("cornersLabel"),d.corners?.home,d.corners?.away);
  html+=matchDetailStatRow(t("shotsOnTargetLabel"),d.shotsOnTarget?.home,d.shotsOnTarget?.away);
  html+=matchDetailStatRow(t("savesLabel"),d.saves?.home,d.saves?.away);
  html+=matchDetailStatRow(t("possessionLabel"),d.possession?.home,d.possession?.away,"%");
  html+=matchDetailStatRow(t("yellowCardsLabel"),d.cards?.home?.yellow,d.cards?.away?.yellow);
  html+=matchDetailStatRow(t("redCardsLabel"),d.cards?.home?.red,d.cards?.away?.red);
  html+='<div class="mdStatRow mdUnavailable"><span class="mdLabel">'+esc(t("throwInsLabel"))+': '+esc(t("noDetailData"))+'</span></div>';
  html+=matchDetailEventList(t("scorersLabel"),d.scorers,x=>'<div class="mdEvent">⚽ '+(x.minute?x.minute+"' · ":"")+esc(x.player||"—")+' <small>('+esc(sideName(x.side))+(x.ownGoal?" · OG":"")+')</small></div>');
  html+=matchDetailEventList(t("penaltiesLabel"),d.penalties,x=>'<div class="mdEvent">'+(x.scored?"⚽":"❌")+' '+(x.minute?x.minute+"' · ":"")+esc(x.player||"—")+' <small>('+esc(sideName(x.side))+')</small></div>');
  html+=matchDetailEventList(t("varLabel"),d.var,x=>'<div class="mdEvent">🖥️ '+(x.minute?x.minute+"' · ":"")+esc(x.detail||"—")+' <small>('+esc(sideName(x.side))+')</small></div>');
  if(!html)html='<div class="mdEmpty">'+t("noDetailData")+'</div>';
  el.innerHTML=html;
  el.classList.add("open");
}
async function toggleMatchDetail(id,rowEl){
  const el=document.getElementById("mdp-"+id);
  if(!el)return;
  if(el.classList.contains("open")){el.classList.remove("open");el.innerHTML="";rowEl?.classList.remove("scoreRowOpen");return;}
  rowEl?.classList.add("scoreRowOpen");
  el.classList.add("open");
  el.innerHTML='<div class="mdEmpty">'+t("loadingDetail")+'</div>';
  const f=(allFixtures||[]).find(x=>String(x.id)===String(id));
  try{
    const r=await fetch("/api/match-detail/"+encodeURIComponent(id),{cache:"no-store"});
    const j=await r.json();
    renderMatchDetail(id,f?.home?.name||"",f?.away?.name||"",j);
  }catch{
    el.innerHTML='<div class="mdEmpty">'+t("noDetailData")+'</div>';
  }
}
