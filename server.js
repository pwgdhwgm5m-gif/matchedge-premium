...
async function tffOfficialFixturesForDate(date){
  ...
  for(const p of pages){
    try{
      const r=await fetch(p.url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.10.5","Accept-Language":"tr-TR,tr;q=0.9"}});
      if(!r.ok)continue;
...
async function swissOfficialFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.10.5","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
      const text=decodeHtmlText(await r.text());
      const dd=date.slice(8,10)+"."+date.slice(5,7)+"."+date.slice(0,4);
...
async function fixturesForThreeDays(centerDate){
  const k=`three-days-v7106:${centerDate}`,cached=getCache(k); if(cached) return cached;
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.10.6-expanded-competitions",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
async function analyzeFixtureModel(f,date){
  const allLeague=await leagueHistory(f.leagueCode),limit=new Date(date+"T00:00:00");
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.10.6 running on port ${PORT}`));
