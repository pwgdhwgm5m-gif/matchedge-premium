...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.7.2-bilingual",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
function dayPickScore(x){
  const pref={["Çifte Şans"]:10,["Gol"]:9,["1X2"]:8,["KG"]:7,["Takım Gol"]:6,["Korner"]:5,["İlk Yarı"]:3,["İkinci Yarı"]:0,["Yarı"]:0};
  return x.probability*.56+x.confidence*.34+(pref[x.group]||0);
}
function chooseDayOptions(model){
  const xs=(model.markets||[])
    .filter(x=>["Çifte Şans","Gol","1X2","KG","Takım Gol","Korner","İlk Yarı","İkinci Yarı"].includes(x.group))
    .filter(x=>x.probability>=57&&x.confidence>=44)
    .filter(x=>x.group!=="İkinci Yarı"||(x.probability>=70&&x.confidence>=52))
    .sort((a,b)=>dayPickScore(b)-dayPickScore(a));
  const out=[];const groups=new Set();
  for(const x of xs){
    if(groups.has(x.group))continue;
    out.push(x);groups.add(x.group);
...
app.get("/api/day-picks",async(req,res)=>{
  try{
    ...
    // One selection per match in coupons. Build three different risk profiles.
    function buildCoupon(maxRisk,size){
      const allowed=maxRisk==="low"?["low"]:maxRisk==="medium"?["low","medium"]:["low","medium","high"];
      const out=[];
      for(const x of results){
        if(!allowed.includes(x.risk)||out.some(y=>y.fixtureId===x.fixtureId))continue;
        out.push(x);if(out.length>=size)break;
      }
      return out;
    }
    const coupons={
      low:buildCoupon("low",2),
      medium:buildCoupon("medium",3),
      high:buildCoupon("high",4)
    };
...
function renderDailyPicks(data){
  ...
  const card=x=>'<div class="pickCard"><b>'+esc(x.home)+' – '+esc(x.away)+' · '+esc(translateMarketName(x.pick))+'</b><span>'+esc(countryName(x.country))+' · '+esc(x.league)+' · '+esc(x.time||"")+'</span><div class="pickMeta"><span>'+t("prob")+': '+x.probability+'%</span><span>'+t("conf")+': '+x.confidence+'%</span><span>'+t("qualityShort")+': '+x.dataQuality+'/100</span></div><div style="font-size:9px;color:#8796a9;margin-top:6px">'+t("modelView")+': xG '+x.expectedGoals.home+'–'+x.expectedGoals.away+' · '+x.likelyScore+'</div></div>';
  let html="";
  ...
  const couponBlock=(title,arr)=>Array.isArray(arr)&&arr.length>=2?'<div class="couponBox"><b>'+title+'</b>'+arr.map((x,i)=>'<div class="couponLeg">'+(i+1)+'. '+esc(x.home)+' – '+esc(x.away)+' · '+esc(translateMarketName(x.pick))+' <span>('+x.probability+'%)</span></div>').join("")+'</div>':"";
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.7.2 running on port ${PORT}`));
