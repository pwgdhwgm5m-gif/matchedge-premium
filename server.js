...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.10.2-api-team-research",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
async function analyzeFixtureModel(f,date){
  ...
  if(!home||!away||!h.length)throw new Error("Takım geçmiş verisi bulunamadı");
  const model=buildModel(h,home,away,leagueH.length?leagueH:h);
  model.researchSource=allLeague.length?"league-history":"api-football-team-history";
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.10.2 running on port ${PORT}`));
