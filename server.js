...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.9.1-scoreboard-sort",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
function fixtureStateRank(f){
  ...
  if(statusFinished(f.status))return 2;
  return 1;
}
...
function renderScoreboard(){
  ...
  const scored=(allFixtures||[]).filter(f=>scoreKnown(f)||isLiveStatus(f.status));
  ...
  scoreboardBody.innerHTML=groups.map(g=>{
    ...
    return '<div class="scoreLeague">'+head+'</div>'+g.arr.map(f=>{
      const live=isLiveStatus(f.status),st=live?t("liveNow")+(f.elapsed?' · '+esc(f.elapsed)+"'":""):statusFinished(f.status)?'FT':esc(f.status||"");
      const sc=scoreKnown(f)?f.score.home+' - '+f.score.away:'–';
      return '<div class="scoreRow"><div>'+esc(f.displayTime||"")+'</div><div class="sh">'+esc(f.home.name)+'</div><div><div class="scoreNum">'+sc+'</div><div class="scoreStatus">'+st+'</div></div><div class="sa">'+esc(f.away.name)+'</div></div>';
    }).join("");
  }).join("");
}
function render(){
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.9.1 running on port ${PORT}`));
