...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.7.1-bilingual",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
function riskTierForPick(model,pick){
  ...
  if(p>=68&&c>=65&&q>=70)return"low";
  if(p>=60&&c>=55&&q>=60)return"medium";
  if(p>=54&&c>=45&&q>=55)return"high";
  return null;
}
...
app.get("/api/day-picks",async(req,res)=>{
  try{
    ...
    const results=[];
    let idx=0;
    const workers=Array.from({length:Math.min(4,fixtures.length||1)},async()=>{
      while(idx<fixtures.length){
        const f=fixtures[idx++];
        try{
          const model=await analyzeFixtureModel(f,date);
          const pick=(model.recommendations||[])[0];
          if(!pick||model.noBet)continue;
          const risk=riskTierForPick(model,pick);
          if(!risk)continue;
          results.push({fixtureId:f.id,leagueCode:f.leagueCode,league:f.league,country:f.country,emoji:f.emoji,home:f.home.name,away:f.away.name,time:f.displayTime,pick:pick.name,probability:pick.probability,confidence:pick.confidence,dataQuality:model.dataQuality,risk});
        }catch{}
      }
    });
    await Promise.all(workers);
    const rank={low:3,medium:2,high:1};
    results.sort((a,b)=>(rank[b.risk]-rank[a.risk])||((b.probability+b.confidence)-(a.probability+a.confidence)));
    const coupon=[];
    for(const x of results){
      if(coupon.length>=3)break;
      if(coupon.some(y=>y.fixtureId===x.fixtureId))continue;
      if(x.risk==="high"&&coupon.length<2)continue;
      coupon.push(x);
    }
    res.json({ok:true,date,count:results.length,picks:results,coupon});
  }catch(e){res.status(500).json({ok:false,error:e.message});}
});
...
function renderDailyPicks(data){
  ...
  const card=x=>'<div class="pickCard"><b>'+esc(x.home)+' – '+esc(x.away)+' · '+esc(translateMarketName(x.pick))+'</b><span>'+esc(countryName(x.country))+' · '+esc(x.league)+' · '+esc(x.time||"")+'</span><div class="pickMeta"><span>'+t("prob")+': '+x.probability+'%</span><span>'+t("conf")+': '+x.confidence+'%</span><span>'+t("qualityShort")+': '+x.dataQuality+'/100</span></div></div>';
  let html="";
  ...
  if((data.coupon||[]).length){html+='<div class="couponBox"><b>'+t("possibleCoupon")+'</b>'+(data.coupon||[]).map((x,i)=>'<div class="couponLeg">'+(i+1)+'. '+esc(x.home)+' – '+esc(x.away)+' · '+esc(translateMarketName(x.pick))+' <span>('+x.probability+'%)</span></div>').join("")+'</div>';}
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.7.1 running on port ${PORT}`));
