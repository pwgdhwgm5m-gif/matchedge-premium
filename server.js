...
async function fixturesForThreeDays(centerDate){
  ...
  let merged=mergeFixtures(apiRows,fdRows).filter(f=>dates.includes(f.localDate));
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.3.0-three-day",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
</style></head><body><div class="app"><div class="top"><div class="brand">MatchEdge <span class="gold">Premium</span></div><div class="live">● LIVE DATA</div></div><div class="hero"><h1>Maçları incele.</h1><p>Dün, bugün ve yarının fikstürleri</p><div class="days" id="days"></div></div><div class="leagues" id="leagues"></div><div class="summary"><div class="stat"><strong id="mc">—</strong><span>SEÇİLİ GÜN MAÇI</span></div><div class="stat"><strong id="ac">0</strong><span>AÇIK ANALİZ</span></div><div class="stat"><strong>3 GÜN</strong><span>DÜN · BUGÜN · YARIN</span></div></div><div class="head"><h2 id="dayHeading">Maçlar</h2><span id="fc"></span></div><div id="fixtures"><div class="loader">Fikstür yükleniyor…</div></div></div><script>
const meta=${leagueMeta};let selectedDate=iso(new Date()),selected="ALL",allFixtures=[],loadSeq=0,openAnalysisId=null;
const esc=s=>String(s??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;");
function iso(x){return x.getFullYear()+"-"+String(x.getMonth()+1).padStart(2,"0")+"-"+String(x.getDate()).padStart(2,"0")}
function addDays(x,n){const d=new Date(x);d.setHours(12,0,0,0);d.setDate(d.getDate()+n);return d}
function statusFinished(s){return["FT","AET","PEN","CANC","ABD","AWD","WO"].includes(s)}
function renderDays(){const today=new Date();today.setHours(12,0,0,0);const defs=[{label:"DÜN",date:addDays(today,-1)},{label:"BUGÜN",date:today},{label:"YARIN",date:addDays(today,1)}];days.innerHTML=defs.map(x=>{const id=iso(x.date);return'<button class="day '+(id===selectedDate?'active':'')+'" data-date="'+id+'"><b>'+x.label+'</b><span>'+x.date.getDate()+'</span><small>'+esc(x.date.toLocaleDateString("tr-TR",{month:"short"}))+'</small></button>'}).join("");days.querySelectorAll("button").forEach(b=>b.onclick=()=>{selectedDate=b.dataset.date;closeOpenAnalysis();renderDays();render();});}
...
function filtered(){return allFixtures.filter(f=>{if(f.localDate!==selectedDate)return false;if(selected!=="ALL"&&f.leagueCode!==selected)return false;const today=iso(new Date());if(selectedDate>today&&statusFinished(f.status))return false;return true;});}
function closeOpenAnalysis(){if(openAnalysisId!==null){const old=document.getElementById("a"+openAnalysisId);if(old)old.innerHTML="";const btn=document.querySelector('.analyze[data-id="'+openAnalysisId+'"]');if(btn)btn.textContent="PRO ANALİZ";}openAnalysisId=null;ac.textContent="0";}
function render(){const a=filtered();mc.textContent=a.length;fc.textContent=a.length+" fikstür";const dd=new Date(selectedDate+"T12:00:00");dayHeading.textContent=dd.toLocaleDateString("tr-TR",{weekday:"long",day:"numeric",month:"long"})+" maçları";if(!a.length){fixtures.innerHTML='<div class="empty">No data</div>';return;}const groups={};a.forEach(f=>(groups[f.leagueCode]??=[]).push(f));fixtures.innerHTML=Object.values(groups).map(g=>'<div class="leagueGroup"><div class="leagueTitle">'+esc(g[0].emoji+" "+g[0].country+" · "+g[0].league)+'</div>'+g.map(f=>'<div class="fixture"><div class="fxhead"><span>'+esc(f.round||f.status||"")+'</span><span>'+esc(f.displayTime||"")+'</span></div><div class="teams"><div class="team">'+(f.home.logo?'<img src="'+esc(f.home.logo)+'" alt="">':'')+esc(f.home.name)+'</div><div class="time">VS</div><div class="team">'+(f.away.logo?'<img src="'+esc(f.away.logo)+'" alt="">':'')+esc(f.away.name)+'</div></div><button class="analyze" data-id="'+f.id+'" onclick="analyze('+f.id+',this)">PRO ANALİZ</button><div id="a'+f.id+'"></div></div>').join("")+'</div>').join("");}
async function loadThreeDays(){const seq=++loadSeq;closeOpenAnalysis();mc.textContent="—";fc.textContent="";fixtures.innerHTML='<div class="loader">Dün, bugün ve yarının fikstürleri yükleniyor…</div>';try{const today=iso(new Date()),r=await fetch("/api/three-days?date="+encodeURIComponent(today)),j=await r.json();if(!j.ok)throw Error(j.error||"Fikstür alınamadı.");if(seq!==loadSeq)return;allFixtures=j.fixtures||[];renderDays();render();}catch(e){if(seq!==loadSeq)return;allFixtures=[];mc.textContent="—";fixtures.innerHTML='<div class="error">Fikstür alınamadı: '+esc(e.message)+'<br><br><button class="analyze" onclick="loadThreeDays()">TEKRAR DENE</button></div>';}}
