...
async function loadNewLeagueCSV(code){
  ...
  try{
    const r=await fetch(`https://www.football-data.co.uk/new/${file}.csv`,{
      headers:{"User-Agent":"MatchEdge/7.14.2","Accept":"text/csv,text/plain,*/*"}
    });
...
async function sportsDbFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"MatchEdge/7.14.2"}});
    if(!r.ok)return[];
    const b=await r.json(),out=[];
    for(const ev of b?.events||[]){
...
async function tffOfficialFixturesForDate(date){
  ...
  for(const p of pages){
    try{
      const raw=await fetchTextSmart(p.url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function tffLeagueHistory(code,date){
  ...
  try{
    const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function swissOfficialFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
  ...
  if(!out.length){
    try{
      const raw=await fetchTextSmart("https://matchcenter-sfl.football.ch/Default.aspx?a=sp&lng=1&ls=25694&oid=2&sg=70075",{"User-Agent":"Mozilla/5.0 MatchEdge/7.14.2"});
...
async function svffOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.svenskfotboll.se/serier-cuper/elitfotboll",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"sv-SE,sv;q=0.9,en;q=0.7"
    });
    const text=decodeHtmlText(raw);
...
async function russianCupOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.rfs.ru/cup/tournament/matches/rpl",{"User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"ru-RU,ru;q=0.9"});
...
function cleanLpfTeam(x){
  return String(x||"").replace(/\b(?:FCSB|FCB|FAR|OSK|UTA|DIN|OGL|UCV|UCJ|FCP|COR|CFR|FCV|FCA|FKCS|RAP)\b/g," ")
    .replace(/\bImage\b/gi," ").replace(/\s+/g," ").trim();
}
async function romanianLeagueOfficialFixturesForDate(date){
  const k=`lpf-official-v1:${date}`,c=getCache(k);if(c)return c;
  const out=[];
  try{
    const raw=await fetchTextSmart("https://lpf.ro/liga-1",{"User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"ro-RO,ro;q=0.9"});
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
    const want=date;
    // LPF renders date/time and both clubs in the round table. Work from date
    // blocks so a changed round number does not break the provider.
    const re=/(\d{1,2})\s+(ian(?:uarie)?|feb(?:ruarie)?|mar(?:tie)?|apr(?:ilie)?|mai|iun(?:ie)?|iul(?:ie)?|aug(?:ust)?|sep(?:tembrie)?|oct(?:ombrie)?|nov(?:iembrie)?|dec(?:embrie)?)\s+(\d{4}),?\s*(\d{1,2}:\d{2})\s+(.{2,100}?)\s+(?:-\s*-|\d+\s+\d+(?:\s+\d+\s+\d+)?)\s+(.{2,100}?)(?=\s+(?:Statistici|\d{1,2}\s+(?:ian|feb|mar|apr|mai|iun|iul|aug|sep|oct|nov|dec))|$)/gi;
    let m;
    while((m=re.exec(text))){
      if(lpfDateToYmd(m[1],m[2],m[3])!==want)continue;
      const home=cleanLpfTeam(m[5]),away=cleanLpfTeam(m[6]);
      if(home.length<2||away.length<2)continue;
      const f=officialFixture("ROU",date,m[4],home,away,"lpf-official");
      if(f)out.push(f);
    }
  }catch(e){console.warn("LPF official fallback:",e.message)}
  const ded=[...new Map(out.map(f=>[fixtureMergeKey(f),f])).values()];
  setCache(k,ded,300000);return ded;
}
...
async function romanianCupOfficialFixturesForDate(date){
  ...
  try{
    ...
    const listing=await fetchTextSmart("https://cuparomaniei.frf.ro/stiri/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"ro-RO,ro;q=0.9"});
    ...
    for(const url of urls){
      let raw;try{raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"ro-RO,ro;q=0.9"})}catch{continue}
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function polishCupFixturesForDate(date){
  ...
  const out=[],p=datePartsForLocale(date);
  try{
    const raw=await fetchTextSmart("https://www.legalsport.pl/rozgrywki/pilka-nozna/puchar-polski/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"pl-PL,pl;q=0.9"});
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function fixturesForThreeDays(centerDate){
  const k=`three-days-v7142:${centerDate}`,cached=getCache(k); if(cached) return cached;
  ...
  // Free/public fallbacks are independent: fetch them concurrently so one
  // slow federation site cannot block the complete three-day feed.
  const fallbackSettled=await Promise.allSettled(dates.flatMap(date=>[
  ...
  Promise.resolve().then(()=>prewarmAnalysisData(fixtures,centerDate))
...
async function apiLeagueResearchHistory(f,date){
  ...
  const season=+String(date||"").slice(0,4);
  const ck=`api-league-history:${leagueId}:${season}:${date}`,cached=getCache(ck);
  if(cached)return cached;
  try{
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.14.2-instant-analysis",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
app.get("/api/data-health",async(req,res)=>{
  try{
    ...
    const result=await fixturesForThreeDays(date);
    const wanted=["TSL","T1L","ROU","ROUC"];
    const leagues={};
    for(const code of wanted){
      const fs=result.fixtures.filter(f=>f.leagueCode===code);
      leagues[code]={fixtures:fs.length,providers:[...new Set(fs.flatMap(f=>f.providers||[f.fixtureSource]).filter(Boolean))]};
    }
    res.json({ok:true,date,leagues});
  }catch(e){res.status(500).json({ok:false,error:e.message})}
});
...
async function romanianLiveScoresForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.gsp.ro/rezultate-live/",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"ro-RO,ro;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
app.get("/api/analyze/:id",async(req,res)=>{
  try{
    ...
    const timeout=new Promise((_,reject)=>setTimeout(()=>reject(new Error("ANALYSIS_TIMEOUT")),4500));
    const model=await Promise.race([analyzeFixtureModel(f,date),timeout]);
    res.json({ok:true,fixture:f,data:{provider:"instant cached model",seasons:["2026/27","2025/26"],h2hRequired:false,analysisMs:Date.now()-started},model});
  }catch(e){
    const msg=e.message==="ANALYSIS_TIMEOUT"?"NO DATA":e.message;
...
async function austriaBundesligaHistory(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.bundesliga.at/de/spielplan",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"de-AT,de;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
    const out=[];
    ...
    if(out.length<6){
      try{
        const fb=await fetchTextSmart("https://fbref.com/en/comps/56/2026-2027/schedule/2026-2027-Austrian-Bundesliga-Scores-and-Fixtures",{
          "User-Agent":"Mozilla/5.0 MatchEdge/7.14.2"
        });
...
async function polandEkstraklasaHistory(date){
  ...
  for(const url of urls){
    try{
      const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.14.2","Accept-Language":"pl-PL,pl;q=0.9,en;q=0.7"});
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function reliableLeagueHistory(f,date,base=[],allowApiFallback=true){
  ...
  if(["TSL","T1L","ROU","ROUC"].includes(code)){
    const jobs=[espnLeagueRangeHistory(f,date)];
    if(["TSL","T1L"].includes(code))jobs.push(tffLeagueHistory(code,date));
    ...
    if(allowApiFallback&&(cov.home<5||cov.away<5)&&API_KEY){
...
async function preparedLeagueHistory(f,date,{allowApiFallback=true}={}){
  ...
  const job=(async()=>{
    ...
    if((code==="AUT2"||code==="TKC")&&!allLeague.length){
      const autH=await austriaBundesligaHistory(date);
      if(autH.length)allLeague=mergeHistoryRows(allLeague,autH);
    }
    setCache(ck,allLeague,900000);
    return allLeague;
...
async function prewarmAnalysisData(fixtures,date){
  const reps=new Map();
  for(const f of fixtures||[]){
    if(f?.localDate!==date)continue;
    if(!reps.has(f.leagueCode))reps.set(f.leagueCode,f);
  }
  const queue=[...reps.values()];
  const workers=Array.from({length:Math.min(3,queue.length)},async()=>{
    while(queue.length){
      const f=queue.shift();
      try{await preparedLeagueHistory(f,date,{allowApiFallback:false});}
      catch(e){console.warn("Analysis prewarm",f?.leagueCode,e.message)}
    }
  });
  await Promise.all(workers);
}
...
function scheduleBackgroundModelEnrichment(f,date){
  const key=`bg-model:${f.id}:${date}`;
  if(backgroundModelJobs.has(key))return;
  const job=(async()=>{
    try{
      let base=await preparedLeagueHistory(f,date,{allowApiFallback:true});
      let limit=new Date(date+"T00:00:00");
      let leagueH=base.filter(x=>x.date<limit);
      let resolved=await resolveTeams(f,leagueH,{allowSiblingLookup:true});
      let home=resolved.home,away=resolved.away,h=resolved.history.filter(x=>x.date<limit);

      // Expensive team-by-team API statistics are allowed only in background.
      if((!home||!away||!h.length)&&API_KEY){
        const researched=await apiTeamResearchHistory(f,date);
        if(researched.length){
          leagueH=mergeHistoryRows(leagueH,researched);
          home=findTeam(f.home.name,leagueH)||f.home.name;
          away=findTeam(f.away.name,leagueH)||f.away.name;
          h=leagueH;
        }
      }
      if(!home||!away||!h.length)return;
      const model=buildModel(h,home,away,leagueH.length?leagueH:h);
      model.researchSource="background-enriched";
      model.researchMatches={home:teamStats(h,home)?.matches||0,away:teamStats(h,away)?.matches||0};
      setCache(`analysis-model:${f.id}:${date}`,model,900000);
    }catch(e){console.warn("Background model enrichment:",e.message)}
  })().finally(()=>backgroundModelJobs.delete(key));
  backgroundModelJobs.set(key,job);
}
...
async function analyzeFixtureModel(f,date){
  ...
  // Cheap cached/public ESPN range is permitted, but no sequential provider chain.
  ...
  if(!leagueH.length){
    // Return quickly instead of blocking the UI for tens of seconds.
    scheduleBackgroundModelEnrichment(f,date);
    throw new Error("NO DATA");
  }

  const model=buildModel(leagueH,home,away,leagueH);
  model.researchSource="instant-cached-history";
  ...
  setCache(modelKey,model,900000);
...
.scoreLeague{margin-top:8px;font-size:9px;font-weight:900;color:#94a3b7;border-bottom:1px solid #ffffff0d;padding:7px 2px}.scoreRow{display:grid;grid-template-columns:48px 1fr auto 1fr;gap:8px;align-items:center;padding:10px 2px;border-bottom:1px solid #ffffff08;font-size:10px}.scoreHome{text-align:right}.scoreCenter{text-align:center;min-width:50px}.scoreMain{font-size:15px;font-weight:950}.scoreStatus{font-size:8px;color:var(--green);font-weight:900}.scoreHt{font-size:8px;color:#8e9aae;margin-top:2px}.scoreEmpty{padding:13px;color:#8190a2;font-size:10px;text-align:center}
...
</style></head><body><div class="goalToast" id="goalToast"></div><div class="shell"><aside class="sideNav"><div class="sideLogo">M</div><button class="sideItem active"><i>⚽</i>Maçlar</button><button class="sideItem"><i>◉</i>Canlı</button><button class="sideItem"><i>▥</i>Analiz</button><button class="sideItem"><i>☆</i>Favoriler</button><div class="sideSpacer"></div><button class="sideItem"><i>⚙</i>Ayarlar</button></aside><div class="mainWrap"><div class="topbar"><div class="brand">MATCHEDGE <span class="gold">PREMIUM</span><small>PRO MATCH INTELLIGENCE</small></div><div class="topActions"><div class="live">● LIVE DATA</div><div class="langSwitch"><button class="langBtn active" id="trBtn" onclick="setLang('tr')" aria-label="Türkçe">🇹🇷</button><button class="langBtn" id="enBtn" onclick="setLang('en')" aria-label="English">🇬🇧</button></div></div></div><div class="app"><div class="hero"><div class="days" id="days"></div></div><div class="leagues" id="leagues"></div><div class="summary"><div class="stat"><strong id="mc">—</strong><span id="sumMatches">SEÇİLİ GÜN MAÇI</span></div><div class="stat"><strong id="ac">0</strong><span id="sumAnalysis">AÇIK ANALİZ</span></div><div class="stat"><strong id="threeDayStrong">3 GÜN</strong><span id="sumDays">DÜN · BUGÜN · YARIN</span></div></div><div class="head"><h2 id="dayHeading">Maçlar</h2><span id="fc"></span></div><details class="scoreboard" id="scoreboard"><summary><span id="scoreboardTitle">⚽ SKOR PANOSU</span><span>⌄</span></summary><div class="scoreboardBody" id="scoreboardBody"></div></details><div id="fixtures"><div class="loader">Fikstür yükleniyor…</div></div><div class="footerNote" id="footerNote"></div><div class="creatorCredit" id="creatorCredit"></div></div></div></div><nav class="mobileNav"><button class="active"><i>⌂</i>Ana Sayfa</button><button><i>⚽</i>Maçlar</button><button><i>▥</i>Analiz</button><button><i>☆</i>Favoriler</button></nav><script>
...
function strongestSignalCard(m){
  ...
  const signal=x
    ? '<span>'+t("strongSignal")+'</span><strong>'+esc(translateMarketName(x.name))+'</strong><small class="signalProb">'+t("signalProbability")+': '+x.probability+'%</small>'
  ...
  const marketToggle='<details class="toggleBox signalMarketToggle"><summary>'+t("marketOddsAnalysis")+'</summary><div class="toggleBody">'+
    '<div class="section">'+t("strong")+'</div>'+marketRows(m.recommendations||[])+
...
function expectedValuesOpen(m,fx,s){
  ...
  return '<details class="expectedOpen expectedToggle">'+
    '<summary class="expectedTop"><div><span>'+t("totalExpectedGoals")+'</span><strong>'+m.expectedGoals.total+'</strong></div><div class="expectedSplit">'+m.expectedGoals.home+' – '+m.expectedGoals.away+'</div></summary>'+
    '<div class="expectedToggleBody"><div class="expectedValuesGrid">'+
...
async function analyze(id,b){
 ...
 try{
  ...
  box.innerHTML='<div class="analysis">'+resultProbabilityCard(m,fx)+visualReadCard(m,fx)+
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.14.2 running on port ${PORT}`));
