...
function applyLiveStateToModel(model,f){
  ...
  model.oddsInfo=null;
  model.noBet=!(strongest&&Number(strongest.analysisProbability??strongest.probability)>=70);
  model.live={
...
async function analyzeFixtureModel(f,date){
  ...
  model.oddsHistorySeries=oddsSeriesForChart(f.apiFixtureId,chartMarket);
...
function strongestSignalCard(m){
  ...
  const marketToggle='<details class="toggleBox signalMarketToggle"><summary>'+t("marketOddsAnalysis")+'</summary><div class="toggleBody">'+
    valueHtml+chartHtml+'<div class="section">'+t("strong")+'</div>'+marketRows(m.recommendations||[])+
