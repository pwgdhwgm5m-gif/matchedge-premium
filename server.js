...
async function fetchFixtureHalftime(f){
  ...
  try{
    const body=await apiFootball(`/fixtures?id=${encodeURIComponent(f.id)}&timezone=${encodeURIComponent(APP_TIME_ZONE)}`,120000);
    const x=body.response?.[0];
    if(!x)return null;
    return {
...
async function enrichHalftimeScores(fixtures){
  ...
  const missing=fixtures.filter(f=>{
    ...
    return htMissing&&(statusFinishedServer(f.status)||["1H","HT","2H","ET","BT","P","INT","LIVE"].includes(String(f.status||"").toUpperCase()));
  });
  let cursor=0;
  const workers=Array.from({length:Math.min(4,missing.length||1)},async()=>{
    while(true){
      ...
      const f=missing[i];
      const d=await fetchFixtureHalftime(f);
      if(!d)continue;
      f.score=f.score||{};
...
async function fixturesForThreeDays(centerDate){
  const k=`three-days-v799:${centerDate}`,cached=getCache(k); if(cached) return cached;
  ...
  await enrichHalftimeScores(fixtures);
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.9.9-halftime-fallback",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.9.9 running on port ${PORT}`));
