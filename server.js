...
async function fixturesForThreeDays(centerDate){
  const k=`three-days-v7104:${centerDate}`,cached=getCache(k); if(cached) return cached;
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.10.4-espn-team-history",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.10.4 running on port ${PORT}`));
