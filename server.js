...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.6.0-bilingual",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
function applyStatic(){
 ...
 sumMatches.textContent=t("selected");sumAnalysis.textContent=t("open");threeDayStrong.textContent=t("three");sumDays.textContent=t("dayLine");footerNote.textContent=t("footer");
...
function render(){
  ...
  meta.forEach(l=>{
    ...
    const country=info?.country||({TSL:"Türkiye",T1L:"Türkiye",TKC:"Türkiye",PL:"İngiltere",CH:"İngiltere",L1:"İngiltere",L2:"İngiltere",NL:"İngiltere",FAC:"İngiltere",PD:"İspanya",SD:"İspanya",CDR:"İspanya",SA:"İtalya",SB:"İtalya",CIT:"İtalya",BL1:"Almanya",BL2:"Almanya",DFB:"Almanya",FL1:"Fransa",FL2:"Fransa",CDF:"Fransa",DED:"Hollanda",KNVB:"Hollanda",BEL:"Belçika",PPL:"Portekiz",GRE:"Yunanistan",SCP:"İskoçya",SCC:"İskoçya",SCL1:"İskoçya",SCL2:"İskoçya",RPL:"Rusya",UPL:"Ukrayna",FIN:"Finlandiya",NOR:"Norveç",SWE:"İsveç",DEN:"Danimarka",SUI:"İsviçre",AUT:"Avusturya",POL:"Polonya",CZE:"Çekya",ROU:"Romanya",CRO:"Hırvatistan",SRB:"Sırbistan",CYP:"Kıbrıs",SVK:"Slovakya",SVN:"Slovenya",ISR:"İsrail",IRL:"İrlanda",UCL:"Avrupa",UEL:"Avrupa",UECL:"Avrupa"})[l.code]||"Diğer";
  ...
  fixtures.innerHTML=Object.entries(countries).map(([country,c])=>{const groups=Object.values(c.leagues);const count=groups.reduce((n,g)=>n+g.matches.length,0);const leaguesHtml=groups.map(g=>{const n=g.matches.length;return'<details class="leagueFold"><summary>'+esc(leagueName(g.meta))+' <span>'+n+' '+(n===1?t("match"):t("matches"))+'</span></summary><div class="leagueMatches">'+(n?g.matches.map(matchHtml).join(""):'<div class="empty">'+t("noGame")+'</div>')+'</div></details>';}).join("");return'<details class="countryGroup"><summary class="countryTitle"><span>'+esc(c.emoji+" "+countryName(country))+'</span><span>'+count+' '+(count===1?t("match"):t("matches"))+'</span></summary><div class="countryBody">'+leaguesHtml+'</div></details>';}).join("");
}
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.6.0 running on port ${PORT}`));
