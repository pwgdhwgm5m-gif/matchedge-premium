...
async function loadCSV(season,div){
  ...
  try{
    for(let attempt=0;attempt<2;attempt++){
      try{
        const r=await fetch(fdUrl(season,div),{headers:{"User-Agent":"MatchEdge/7.18.4","Accept":"text/csv,text/plain,*/*"}});
...
async function loadNewLeagueCSV(code){
  ...
  try{
    const r=await fetch(`https://www.football-data.co.uk/new/${file}.csv`,{
      headers:{"User-Agent":"MatchEdge/7.18.4","Accept":"text/csv,text/plain,*/*"}
    });
  ...
  if(withOdds.length){
    const ranked=withOdds
      ...
      .filter(x=>{
        ...
        const strongProb=p>=70;
        const valueCase=p>=58&&Number(x.edge)>=4&&Number(x.expectedValue)>=5;
        return c>=45&&(strongProb||valueCase)&&Number(x.edge)>=0&&Number(x.expectedValue)>=0;
      })
      .map(x=>{
        const p=Number(x.analysisProbability??x.probability);
...
function chooseStrongestSignal(markets=[],betRecommendation=null){
  const priced=(markets||[]).filter(x=>Number(x.marketOdd)>1);
  if(priced.length){
    const playable=priced.filter(x=>{
      const p=Number(x.analysisProbability??x.probability)||0;
      const c=Number(x.analysisConfidence??x.confidence)||0;
      const odd=Number(x.marketOdd);
      const ev=Number(x.expectedValue);
      const edge=Number(x.edge);
      return odd>=1.28 && p>=70 && c>=45 &&
        (!Number.isFinite(ev)||ev>=0) &&
        (!Number.isFinite(edge)||edge>=0);
    }).sort((a,b)=>strongestSignalScore(b)-strongestSignalScore(a));

    if(playable[0]){
      return {...playable[0],decision:betRecommendation?.name===playable[0].name&&betRecommendation?.decision==="BET"?"BET":"PLAYABLE"};
    }

    const ranked=priced.slice().sort((a,b)=>strongestSignalScore(b)-strongestSignalScore(a));
    return ranked[0]?{...ranked[0],decision:"MODEL LEAN"}:null;
  }

  const modelOnly=(markets||[]).slice().sort((a,b)=>strongestSignalScore(b)-strongestSignalScore(a));
  return modelOnly[0]?{...modelOnly[0],decision:"MODEL LEAN"}:null;
}
...
function buildModel(h,home,away,leagueH=h,fixtureOdds=null){
  ...
  const strongestOdd=Number(strongest?.marketOdd);
  const strongestEV=Number(strongest?.expectedValue);

  // NO BET is stricter than "strongest signal": a good model signal can exist without a worthwhile price.
  const noBet=betRecommendation?.decision==="BET"
    ? false
    : !(strongest && strongestP>=70 && strongestC>=45 &&
        (!Number.isFinite(strongestOdd) || strongestOdd>=1.28) &&
        (!Number.isFinite(strongestEV) || strongestEV>=0));
...
async function sportsDbFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"MatchEdge/7.18.4"}});
    if(!r.ok)return[];
    const b=await r.json(),out=[];
    for(const ev of b?.events||[]){
...
async function tffOfficialFixturesForDate(date){
  ...
  for(const p of pages){
    try{
      const raw=await fetchTextSmart(p.url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.4","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function tffLeagueHistory(code,date){
  ...
  try{
    const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.4","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function swissOfficialFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.4","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
  ...
  if(!out.length){
    try{
      const raw=await fetchTextSmart("https://matchcenter-sfl.football.ch/Default.aspx?a=sp&lng=1&ls=25694&oid=2&sg=70075",{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.4"});
...
async function svffOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.svenskfotboll.se/serier-cuper/elitfotboll",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.18.4","Accept-Language":"sv-SE,sv;q=0.9,en;q=0.7"
    });
    const text=decodeHtmlText(raw);
...
async function russianCupOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.rfs.ru/cup/tournament/matches/rpl",{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.4","Accept-Language":"ru-RU,ru;q=0.9"});
...
async function polishCupFixturesForDate(date){
  ...
  const out=[],p=datePartsForLocale(date);
  try{
    const raw=await fetchTextSmart("https://www.legalsport.pl/rozgrywki/pilka-nozna/puchar-polski/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.4","Accept-Language":"pl-PL,pl;q=0.9"});
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function fixturesForThreeDays(centerDate){
  const k=`daily-v7184:${centerDate}`,cached=getCache(k); if(cached) return cached;
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.18.4-analysis-runtime-fix",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:false,timezoneNormalization:true,fixtureMerge:true,romaniaStructuredOnly:true,analysisToggle:true,marketAwareAnalysis:true,noVigMarketProbability:true}}));
...
async function romanianLiveScoresForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.gsp.ro/rezultate-live/",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.18.4","Accept-Language":"ro-RO,ro;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function austriaBundesligaHistory(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.bundesliga.at/de/spielplan",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.18.4","Accept-Language":"de-AT,de;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
    const out=[];
    ...
    if(out.length<6){
      try{
        const fb=await fetchTextSmart("https://fbref.com/en/comps/56/2026-2027/schedule/2026-2027-Austrian-Bundesliga-Scores-and-Fixtures",{
          "User-Agent":"Mozilla/5.0 MatchEdge/7.18.4"
        });
...
async function polandEkstraklasaHistory(date){
  ...
  for(const url of urls){
    try{
      const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.18.4","Accept-Language":"pl-PL,pl;q=0.9,en;q=0.7"});
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
function strongestSignalCard(m){
  ...
  const signal=x
    ? '<span>'+t("strongSignal")+'</span><strong>'+esc(translateMarketName(x.name))+
      (x.decision==="BET"?' · BET':x.decision==="PLAYABLE"?' · OYNANABİLİR':'')+
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.18.4 running on port ${PORT}`));
