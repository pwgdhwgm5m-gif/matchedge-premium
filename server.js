...
async function loadNewLeagueCSV(code){
  ...
  try{
    const r=await fetch(`https://www.football-data.co.uk/new/${file}.csv`,{
      headers:{"User-Agent":"MatchEdge/7.12.3","Accept":"text/csv,text/plain,*/*"}
    });
...
async function sportsDbFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"MatchEdge/7.12.3"}});
    if(!r.ok)return[];
    const b=await r.json(),out=[];
    for(const ev of b?.events||[]){
...
async function tffOfficialFixturesForDate(date){
  ...
  for(const p of pages){
    try{
      const raw=await fetchTextSmart(p.url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function tffLeagueHistory(code,date){
  ...
  try{
    const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function swissOfficialFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
  ...
  if(!out.length){
    try{
      const raw=await fetchTextSmart("https://matchcenter-sfl.football.ch/Default.aspx?a=sp&lng=1&ls=25694&oid=2&sg=70075",{"User-Agent":"Mozilla/5.0 MatchEdge/7.12.3"});
...
async function svffOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.svenskfotboll.se/serier-cuper/elitfotboll",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"sv-SE,sv;q=0.9,en;q=0.7"
    });
    const text=decodeHtmlText(raw);
...
async function russianCupOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.rfs.ru/cup/tournament/matches/rpl",{"User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"ru-RU,ru;q=0.9"});
...
async function romanianCupOfficialFixturesForDate(date){
  ...
  try{
    ...
    const listing=await fetchTextSmart("https://cuparomaniei.frf.ro/stiri/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"ro-RO,ro;q=0.9"});
    ...
    for(const url of urls){
      let raw;try{raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"ro-RO,ro;q=0.9"})}catch{continue}
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function polishCupFixturesForDate(date){
  ...
  const out=[],p=datePartsForLocale(date);
  try{
    const raw=await fetchTextSmart("https://www.legalsport.pl/rozgrywki/pilka-nozna/puchar-polski/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"pl-PL,pl;q=0.9"});
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function fetchHistoryHalftime(f){
  // FIX: leagues sourced from the new-leagues single-file CSV (POL/SWE/SUI/RPL)
  // have real leagueHistory() data now too, so they should not be excluded here.
  if(!f||(!LEAGUES[f.leagueCode]?.csv&&!NEW_LEAGUE_CSV[f.leagueCode]))return null;
  try{
...
async function enrichHalftimeScores(fixtures){
  ...
  for(const f of fixtures){
    ...
    // FIX: also allow the new-leagues single-file source (POL/SWE/SUI/RPL) through,
    // not just the classic mmz4281 csv leagues.
    if(!LEAGUES[f.leagueCode]?.csv&&!NEW_LEAGUE_CSV[f.leagueCode])continue;
...
async function fixturesForThreeDays(centerDate){
  const k=`three-days-v7123:${centerDate}`,cached=getCache(k); if(cached) return cached;
...
async function resolveTeams(f,leagueH){
  ...
  if(!home||!away){
    ...
    // FIX: only widen the search to "every classic-CSV league" when this league truly
    // has no history source at all. POL/SWE/SUI/RPL now have their own correct history
    // via NEW_LEAGUE_CSV, so they must not fall into this unrelated-leagues net.
    if(!LEAGUES[f.leagueCode]?.csv&&!NEW_LEAGUE_CSV[f.leagueCode]) sib=Object.keys(LEAGUES).filter(c=>LEAGUES[c].csv||NEW_LEAGUE_CSV[c]);
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.12.3-extra-league-csv-history",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
async function romanianLiveScoresForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.gsp.ro/rezultate-live/",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"ro-RO,ro;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function austriaBundesligaHistory(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.bundesliga.at/de/spielplan",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"de-AT,de;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
    const out=[];
    ...
    if(out.length<6){
      try{
        const fb=await fetchTextSmart("https://fbref.com/en/comps/56/2026-2027/schedule/2026-2027-Austrian-Bundesliga-Scores-and-Fixtures",{
          "User-Agent":"Mozilla/5.0 MatchEdge/7.12.3"
        });
...
async function polandEkstraklasaHistory(date){
  ...
  for(const url of urls){
    try{
      const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.12.3","Accept-Language":"pl-PL,pl;q=0.9,en;q=0.7"});
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function analyzeFixtureModel(f,date){
  ...
  if(f.leagueCode==="AUT"){
    // FIX: same replace-vs-merge issue as the Poland block above.
    const autH=await austriaBundesligaHistory(date);
    if(autH.length){
      const seen=new Set(allLeague.map(x=>`${localYmdFromDate(x.date)}|${norm(x.home)}|${norm(x.away)}`));
      const extra=autH.filter(x=>!seen.has(`${localYmdFromDate(x.date)}|${norm(x.home)}|${norm(x.away)}`));
      if(extra.length)allLeague=[...allLeague,...extra].sort((a,b)=>a.date-b.date);
    }
  }
  if(f.leagueCode==="POL"){
    // FIX: this live scraper only ever returns current-season ("2026/27") rows, and
    // previously it fully REPLACED allLeague (which now correctly holds both the
    // 2025/26 and 2026/27 seasons from football-data.co.uk via NEW_LEAGUE_CSV). That
    // wiped out the prior-season support data teamStats()/weightedAvg() rely on early
    // in the season. Merge instead of replace so we keep both.
    const polH=await polandEkstraklasaHistory(date);
    if(polH.length){
      const seen=new Set(allLeague.map(x=>`${localYmdFromDate(x.date)}|${norm(x.home)}|${norm(x.away)}`));
      const extra=polH.filter(x=>!seen.has(`${localYmdFromDate(x.date)}|${norm(x.home)}|${norm(x.away)}`));
      if(extra.length)allLeague=[...allLeague,...extra].sort((a,b)=>a.date-b.date);
    }
  }
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.12.3 running on port ${PORT}`));
