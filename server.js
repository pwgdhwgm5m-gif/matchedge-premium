...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.9.5-scoreboard-detail",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
function renderScoreboard(){
  ...
  scoreboardBody.innerHTML=groups.map(g=>{
    ...
    return '<div class="scoreLeague">'+head+'</div>'+g.arr.map(f=>{
      ...
      const detail=(live||finished)?'<div class="scoreDetail">'+
        (htKnown?'<span class="scorePill">'+t("htShort")+' '+f.score.htHome+'-'+f.score.htAway+'</span>':'')+
        (scoreKnown(f)?'<span class="scorePill">'+t("ftShort")+' '+f.score.home+'-'+f.score.away+'</span>':'')+
        '</div>':"";
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.9.5 running on port ${PORT}`));
