...
async function sportsDbFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"MatchEdge/7.11.1"}});
    if(!r.ok)return[];
    const b=await r.json(),out=[];
    for(const ev of b?.events||[]){
...
async function tffOfficialFixturesForDate(date){
  ...
  for(const p of pages){
    try{
      const raw=await fetchTextSmart(p.url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.11.1","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function tffLeagueHistory(code,date){
  ...
  try{
    const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.11.1","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function swissOfficialFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.11.1","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
  ...
  if(!out.length){
    try{
      const raw=await fetchTextSmart("https://matchcenter-sfl.football.ch/Default.aspx?a=sp&lng=1&ls=25694&oid=2&sg=70075",{"User-Agent":"Mozilla/5.0 MatchEdge/7.11.1"});
...
async function svffOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.svenskfotboll.se/serier-cuper/elitfotboll",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.11.1","Accept-Language":"sv-SE,sv;q=0.9,en;q=0.7"
    });
...
async function russianCupOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.rfs.ru/cup/tournament/matches/rpl",{"User-Agent":"Mozilla/5.0 MatchEdge/7.11.1","Accept-Language":"ru-RU,ru;q=0.9"});
...
async function romanianCupOfficialFixturesForDate(date){
  ...
  try{
    ...
    const listing=await fetchTextSmart("https://cuparomaniei.frf.ro/stiri/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.11.1","Accept-Language":"ro-RO,ro;q=0.9"});
    ...
    for(const url of urls){
      let raw;try{raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.11.1","Accept-Language":"ro-RO,ro;q=0.9"})}catch{continue}
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function polishCupFixturesForDate(date){
  ...
  const out=[],p=datePartsForLocale(date);
  try{
    const raw=await fetchTextSmart("https://www.legalsport.pl/rozgrywki/pilka-nozna/puchar-polski/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.11.1","Accept-Language":"pl-PL,pl;q=0.9"});
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function fixturesForThreeDays(centerDate){
  const k=`three-days-v7111:${centerDate}`,cached=getCache(k); if(cached) return cached;
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.11.1-sportsdb-global-fallback",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.11.1 running on port ${PORT}`));
