...
async function loadNewLeagueCSV(code){
  ...
  try{
    const r=await fetch(`https://www.football-data.co.uk/new/${file}.csv`,{
      headers:{"User-Agent":"MatchEdge/7.13.2","Accept":"text/csv,text/plain,*/*"}
    });
...
async function sportsDbFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"MatchEdge/7.13.2"}});
    if(!r.ok)return[];
    const b=await r.json(),out=[];
    for(const ev of b?.events||[]){
...
async function tffOfficialFixturesForDate(date){
  ...
  for(const p of pages){
    try{
      const raw=await fetchTextSmart(p.url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function tffLeagueHistory(code,date){
  ...
  try{
    const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"tr-TR,tr;q=0.9"});
...
async function swissOfficialFixturesForDate(date){
  ...
  try{
    ...
    const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"de-CH,de;q=0.9"}});
    if(r.ok){
  ...
  if(!out.length){
    try{
      const raw=await fetchTextSmart("https://matchcenter-sfl.football.ch/Default.aspx?a=sp&lng=1&ls=25694&oid=2&sg=70075",{"User-Agent":"Mozilla/5.0 MatchEdge/7.13.2"});
...
async function svffOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.svenskfotboll.se/serier-cuper/elitfotboll",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"sv-SE,sv;q=0.9,en;q=0.7"
    });
    const text=decodeHtmlText(raw);
...
async function russianCupOfficialFixturesForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.rfs.ru/cup/tournament/matches/rpl",{"User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"ru-RU,ru;q=0.9"});
...
async function romanianCupOfficialFixturesForDate(date){
  ...
  try{
    ...
    const listing=await fetchTextSmart("https://cuparomaniei.frf.ro/stiri/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"ro-RO,ro;q=0.9"});
    ...
    for(const url of urls){
      let raw;try{raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"ro-RO,ro;q=0.9"})}catch{continue}
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function polishCupFixturesForDate(date){
  ...
  const out=[],p=datePartsForLocale(date);
  try{
    const raw=await fetchTextSmart("https://www.legalsport.pl/rozgrywki/pilka-nozna/puchar-polski/",{"User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"pl-PL,pl;q=0.9"});
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function fixturesForThreeDays(centerDate){
  const k=`three-days-v7132:${centerDate}`,cached=getCache(k); if(cached) return cached;
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.13.2-focused-match-read",timezone:APP_TIME_ZONE,mode:MODE,providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true},features:{yesterdayTodayTomorrow:true,timezoneNormalization:true,fixtureMerge:true,analysisToggle:true}}));
...
async function romanianLiveScoresForDate(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.gsp.ro/rezultate-live/",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"ro-RO,ro;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
async function austriaBundesligaHistory(date){
  ...
  try{
    const raw=await fetchTextSmart("https://www.bundesliga.at/de/spielplan",{
      "User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"de-AT,de;q=0.9"
    });
    const text=decodeHtmlText(raw).replace(/\s+/g," ");
    const out=[];
    ...
    if(out.length<6){
      try{
        const fb=await fetchTextSmart("https://fbref.com/en/comps/56/2026-2027/schedule/2026-2027-Austrian-Bundesliga-Scores-and-Fixtures",{
          "User-Agent":"Mozilla/5.0 MatchEdge/7.13.2"
        });
...
async function polandEkstraklasaHistory(date){
  ...
  for(const url of urls){
    try{
      const raw=await fetchTextSmart(url,{"User-Agent":"Mozilla/5.0 MatchEdge/7.13.2","Accept-Language":"pl-PL,pl;q=0.9,en;q=0.7"});
      const text=decodeHtmlText(raw).replace(/\s+/g," ");
...
.summaryCard{padding:15px;border:1px solid #1f3a50;border-radius:15px;background:#0a1b2a;min-height:100px}.summaryCard span{display:block;font-size:8px;color:#8493a5;letter-spacing:.4px}.summaryCard strong{display:block;margin-top:8px;font-size:19px}.summaryCard small{display:block;margin-top:6px;font-size:8px;color:#8392a4}
.signalCard{border-color:#18d5b044;background:linear-gradient(145deg,#0d2a2d,#0b1d27)}.signalCard strong{color:var(--gold);font-size:17px}.signalProb{color:var(--teal)!important;font-weight:900}
...
@media(max-width:760px){
 ...
 .grid{grid-template-columns:repeat(2,1fr)}.readSub{grid-template-columns:repeat(2,1fr)}.readRow{grid-template-columns:88px 1fr 42px}.focusGrid{grid-template-columns:repeat(2,1fr)}.focusSummary{grid-template-columns:1fr 1fr}.signalCard{grid-column:1/-1}.focusMetric{min-height:105px}
...
function strongestSignalCard(m){
  ...
  if(!x){
    return '<div class="summaryCard signalCard"><span>'+t("strongSignal")+'</span><strong>NO BET</strong><small>'+t("noBet")+'</small></div>';
  }
  return '<div class="summaryCard signalCard"><span>'+t("strongSignal")+'</span><strong>'+esc(translateMarketName(x.name))+'</strong><small class="signalProb">'+t("signalProbability")+': '+x.probability+'%</small></div>';
}
...
async function analyze(id,b){
 ...
 try{
  ...
  const other='<details class="otherAnalysis"><summary>'+t("otherAnalysis")+'</summary><div class="otherBody">'+
    '<div class="grid"><div class="mini"><span>'+t("likelyScore")+'</span><strong>'+m.likelyScore+'</strong></div><div class="mini"><span>'+t("corners")+'</span><strong>'+(s.expectedCorners??"—")+'</strong></div></div>'+
    teamStats+
    '<div class="section">'+t("strong")+'</div>'+marketRows(m.recommendations)+
    '<details class="toggleBox"><summary>'+t("basis")+'</summary><div class="toggleBody">'+(m.reasons||[]).map(x=>'<div class="reason">• '+esc(translateReason(x))+'</div>').join("")+'</div></details>'+
    '<div class="section">'+t("allMarkets")+'</div>'+allMarketsBlock(m.markets,m)+
    '</div></details>';
  box.innerHTML='<div class="analysis">'+visualReadCard(m,fx)+
    '<div class="focusSummary">'+
      '<div class="summaryCard"><span>'+t("totalExpectedGoals")+'</span><strong>'+m.expectedGoals.total+'</strong><small>'+m.expectedGoals.home+' – '+m.expectedGoals.away+'</small></div>'+
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.13.2 running on port ${PORT}`));
