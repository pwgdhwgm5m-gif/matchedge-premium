...
/* =========================================================
   MATCHEDGE PREMIUM V7.2.2 PROFESSIONAL
   Fixtures: API-Football + Football-Data.co.uk fallback
...
const LEAGUE_BY_CSV = {};
for (const [code,l] of Object.entries(LEAGUES)) LEAGUE_BY_CSV[l.csv] = {code,...l};
...
async function loadCSV(season,div){
  ...
  try{const r=await fetch(fdUrl(season,div),{headers:{"User-Agent":"MatchEdge/7.2"}});if(!r.ok)return[];const text=await r.text();if(text.toLowerCase().includes("<html")||text.length<20)return[];const rows=parseCSV(text);setCache(k,rows,1800000);return rows;}catch{return[];}
}
...
async function footballDataUpcoming(date){
  const k=`fd-upcoming:${date}`,c=getCache(k);if(c)return c;
  try{
    const c=new AbortController();const timer=setTimeout(()=>c.abort(),9000);
    let r;try{r=await fetch(FD_FIXTURES_URL,{headers:{"User-Agent":"MatchEdge/7.2.5"},signal:c.signal});}finally{clearTimeout(timer);}
    if(!r.ok)throw new Error(`Football-Data fixtures HTTP ${r.status}`);
    const text=await r.text();if(text.toLowerCase().includes("<html")||text.length<20)throw new Error("Football-Data fixtures yanıtı geçersiz.");
    ...
    for(const row of rows){
      ...
      const f={id,date:dt,timestamp:Math.floor(new Date(dt).getTime()/1000),status:"NS",leagueCode:l.code,league:l.name,country:l.country,emoji:l.emoji,round:"",home:{name:home,logo:""},away:{name:away,logo:""},fixtureSource:"football-data.co.uk"};
      out.push(f);setCache(`fixture:${id}`,f,21600000);
    }
    out.sort((a,b)=>a.timestamp-b.timestamp);setCache(k,out,21600000);return out;
  }catch(e){throw new Error(`Football-Data fikstürü alınamadı: ${e.message}`);}
}


async function footballDataSeasonUpcoming(date){
  const k=`fd-season-upcoming:${date}`,c=getCache(k);if(c)return c;
  const out=[];
  try{
    const entries=Object.entries(LEAGUES);
    const rowsByLeague=await Promise.all(entries.map(async([code,l])=>[code,l,await loadCSV("2627",l.csv)]));
    for(const [code,l,rows] of rowsByLeague){
      for(const row of rows){
        const d=parseDate(row.Date||row.date);if(!d||ymd(d)!==date)continue;
        const home=String(row.HomeTeam||row.Home||"").trim(),away=String(row.AwayTeam||row.Away||"").trim();
        if(!home||!away)continue;
        if(num(row.FTHG)!=null&&num(row.FTAG)!=null)continue;
        let tm=String(row.Time||row.time||"12:00").trim();if(!/^\\d{1,2}:\\d{2}$/.test(tm))tm="12:00";if(tm.length===4)tm="0"+tm;
        const dt=`${date}T${tm}:00`,id=stableFixtureId([l.csv,date,home,away,"season"]);
        const f={id,date:dt,timestamp:Math.floor(new Date(dt).getTime()/1000),status:"NS",leagueCode:code,league:l.name,country:l.country,emoji:l.emoji,round:"",home:{name:home,logo:""},away:{name:away,logo:""},fixtureSource:"football-data.co.uk-season"};
        out.push(f);setCache(`fixture:${id}`,f,21600000);
      }
    }
  }catch(e){return []}
...
/* ---------------- Fixtures + resilient team history ---------------- */
function mapFixture(x){const l=LEAGUE_BY_API[x?.league?.id];if(!l)return null;return{id:x.fixture.id,date:x.fixture.date,timestamp:x.fixture.timestamp,status:x.fixture.status?.short||"",leagueCode:l.code,league:l.name,country:l.country,emoji:l.emoji,round:x.league.round||"",home:{name:x.teams.home.name,logo:x.teams.home.logo||""},away:{name:x.teams.away.name,logo:x.teams.away.logo||""}};}
async function fixturesForDate(date){
  const today=new Date().toISOString().slice(0,10),ttl=date>today?21600000:900000;
  try{
    const rows=await apiFootball(`/fixtures?date=${encodeURIComponent(date)}`,ttl);
    const out=rows.map(mapFixture).filter(Boolean).sort((a,b)=>a.timestamp-b.timestamp);
    out.forEach(f=>setCache(`fixture:${f.id}`,f,3600000));
    if(out.length)return out;
  }catch(e){ /* API plan/date errors are not user-facing */ }
  try{
    const fd=await footballDataUpcoming(date);
    if(fd.length)return fd;
  }catch(e){ /* silent fallback */ }
  return [];
}
async function fixturesForRange(from,to){
  const k=`range:${from}:${to}`,c=getCache(k);if(c)return c;
  const start=new Date(from+"T12:00:00"),end=new Date(to+"T12:00:00"),all=[];
  for(let d=new Date(start);d<=end;d.setDate(d.getDate()+1)){
    const ds=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")}`;
    all.push(...await fixturesForDate(ds));
  }
  all.sort((a,b)=>a.timestamp-b.timestamp);setCache(k,all,21600000);return all;
}
function siblingLeagueCodes(code){const base=LEAGUES[code];return Object.entries(LEAGUES).filter(([c,l])=>c!==code&&l.country===base.country).map(([c])=>c);}
async function resolveTeams(f,leagueH){
  let home=findTeam(f.home.name,leagueH),away=findTeam(f.away.name,leagueH),extra=[];
  if(!home||!away){const sib=siblingLeagueCodes(f.leagueCode);const hs=await Promise.all(sib.map(leagueHistory));extra=hs.flat();if(!home)home=findTeam(f.home.name,extra);if(!away)away=findTeam(f.away.name,extra);}
  const merged=[...leagueH,...extra].sort((a,b)=>a.date-b.date);
  return {home,away,history:merged};
}

/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.2.5-professional",providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true,odds:false},features:{weeklyCalendar:true,leaguePerformance:true,corners:"9.5-11.5 collapsible",h2hOptional:true}}));
...
app.get("/api/week",async(req,res)=>{try{const from=String(req.query.from||"");if(!/^\d{4}-\d{2}-\d{2}$/.test(from))return res.status(400).json({ok:false,error:"Geçerli hafta başlangıcı gerekli."});const d=new Date(from+"T12:00:00");const z=new Date(d);z.setDate(z.getDate()+6);const to=`${z.getFullYear()}-${String(z.getMonth()+1).padStart(2,"0")}-${String(z.getDate()).padStart(2,"0")}`;const fixtures=await fixturesForRange(from,to);res.json({ok:true,from,to,count:fixtures.length,fixtures});}catch(e){res.status(500).json({ok:false,error:e.message});}});
app.get("/api/analyze/:id",async(req,res)=>{try{
  const date=String(req.query.date||""),id=+req.params.id;if(!/^\d{4}-\d{2}-\d{2}$/.test(date))return res.status(400).json({ok:false,error:"Geçerli tarih gerekli."});
  let f=getCache(`fixture:${id}`);if(!f){const fs=await fixturesForDate(date);f=fs.find(x=>x.id===id);}if(!f)return res.status(404).json({ok:false,error:"Maç bulunamadı."});
...
*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at top,#12233a,#07111f 46%);color:#f4f7fb;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.app{max-width:760px;margin:auto;padding:16px 13px 70px}.top{display:flex;justify-content:space-between;align-items:center}.brand{font-weight:850;font-size:19px}.gold{color:#d9b56f}.live{font-size:10px;color:#48d89b;background:#48d89b18;padding:7px 10px;border-radius:99px}.hero{margin-top:16px;padding:19px;border:1px solid #d9b56f33;border-radius:23px;background:linear-gradient(145deg,#d9b56f18,#122239bb)}.hero small{color:#d9b56f;font-weight:800;font-size:10px}.hero h1{margin:7px 0 4px;font-size:27px}.hero p{color:#91a0b3;font-size:12px;line-height:1.45}.weeknav{display:grid;grid-template-columns:42px 1fr 42px;gap:8px;margin-top:16px}.weeknav button,.weektitle{height:40px;border:1px solid #ffffff14;background:#ffffff08;color:white;border-radius:13px;display:flex;align-items:center;justify-content:center}.weektitle{font-size:11px;color:#d9b56f;font-weight:800}.days{display:grid;grid-template-columns:repeat(7,minmax(66px,1fr));gap:7px;overflow-x:auto;padding:12px 1px 2px}.day{min-width:66px;border:1px solid #ffffff14;background:#ffffff07;color:#aeb9c8;border-radius:14px;padding:9px 5px;text-align:center}.day b{display:block;font-size:11px}.day span{display:block;font-size:15px;color:white;margin-top:3px}.day.active{background:#d9b56f;color:#07111f;border-color:#d9b56f}.day.active span{color:#07111f}.leagues{display:flex;gap:8px;overflow:auto;padding:14px 0 10px}.chip{white-space:nowrap;border:1px solid #ffffff14;background:#ffffff08;color:#aeb9c8;padding:9px 12px;border-radius:99px}.chip.active{background:#d9b56f;color:#07111f}.summary{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:4px 0 18px}.stat{text-align:center;padding:12px;border:1px solid #ffffff14;border-radius:15px}.stat strong{display:block;font-size:17px}.stat span{font-size:9px;color:#8492a6}.head{display:flex;justify-content:space-between;align-items:end}.head h2{font-size:17px;margin:8px 0}.head span{font-size:10px;color:#8492a6}.leagueGroup{margin:14px 0 20px}.leagueTitle{font-size:12px;font-weight:850;color:#d9b56f;margin:8px 3px}.fixture{padding:15px;margin:9px 0;border:1px solid #ffffff14;border-radius:19px;background:#0d1a2bea}.fxhead{display:flex;justify-content:space-between;color:#8492a6;font-size:9px}.teams{display:grid;grid-template-columns:1fr 52px 1fr;align-items:center;margin-top:13px}.team{text-align:center;font-size:12px;font-weight:750}.team img{width:38px;height:38px;object-fit:contain;display:block;margin:0 auto 6px}.time{text-align:center;color:#d9b56f;font-weight:850}.analyze{width:100%;height:43px;margin-top:13px;border:0;border-radius:13px;background:linear-gradient(135deg,#d9b56f,#f2d18f);font-weight:900;color:#07111f}.analysis{border-top:1px solid #ffffff14;margin-top:13px;padding-top:13px}.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.mini{padding:11px;border-radius:12px;background:#ffffff08}.mini span{display:block;color:#8492a6;font-size:8px}.mini strong{font-size:14px}.teamstate{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.state{background:#ffffff06;border:1px solid #ffffff0d;border-radius:13px;padding:11px}.state b{font-size:11px;display:block;margin-bottom:5px}.state span{display:block;color:#9ba8b8;font-size:9px;line-height:1.55}.section{margin-top:15px;color:#d9b56f;font-size:10px;font-weight:850;letter-spacing:.35px}.market{display:grid;grid-template-columns:1fr 64px;padding:10px 0;border-bottom:1px solid #ffffff0d;font-size:12px}.prob{color:#48d89b;font-weight:850;text-align:right}.cornerBox{margin-top:14px;border:1px solid #d9b56f2e;border-radius:14px;overflow:hidden}.cornerBox summary{cursor:pointer;list-style:none;padding:14px 12px;font-size:11px;font-weight:900;color:#d9b56f;background:#d9b56f0a}.cornerBox summary::-webkit-details-marker{display:none}.cornerBox summary:after{content:'＋';float:right;color:#d9b56f}.cornerBox[open] summary:after{content:'−'}.cornerBody{padding:0 12px 7px}.reason{font-size:10px;color:#aeb9c8;padding:5px 0;line-height:1.45}.warn,.error,.empty,.nobet{margin-top:10px;padding:13px;border-radius:13px;font-size:10px;line-height:1.45}.warn{color:#efc987;background:#d9b56f12}.error{color:#ff9aa5;background:#ff708014}.empty{text-align:center;color:#8492a6;border:1px dashed #ffffff1f}.nobet{color:#ffc66d;background:#ffc66d12;border:1px solid #ffc66d22;font-weight:850}.loader{text-align:center;color:#8492a6;padding:24px}
</style></head><body><div class="app"><div class="top"><div class="brand">MatchEdge <span class="gold">Premium</span></div><div class="live">● LIVE DATA · 7.2.5</div></div><div class="hero"><h1>Maçları haftalık incele.</h1><div class="weeknav"><button id="prevWeek">‹</button><div class="weektitle" id="weekTitle"></div><button id="nextWeek">›</button></div><div class="days" id="days"></div></div><div class="leagues" id="leagues"></div><div class="summary"><div class="stat"><strong id="mc">—</strong><span>SEÇİLİ GÜN MAÇI</span></div><div class="stat"><strong id="ac">0</strong><span>ANALİZ</span></div><div class="stat"><strong>7 GÜN</strong><span>HAFTALIK TAKVİM</span></div></div><div class="head"><h2 id="dayHeading">Maçlar</h2><span id="fc"></span></div><div id="fixtures"><div class="loader">Seçili gün fikstürü yükleniyor…</div></div></div><script>
const meta=${leagueMeta};
const $=id=>document.getElementById(id);
const daysEl=$("days"),leaguesEl=$("leagues"),weekTitleEl=$("weekTitle"),mcEl=$("mc"),acEl=$("ac"),fcEl=$("fc"),headingEl=$("dayHeading"),fixturesEl=$("fixtures");
let now=new Date(),weekStart=startMonday(now),selectedDate=iso(now),selected="ALL",weekFixtures=[],analyses=0,dayCache={},loadSeq=0;
const esc=s=>String(s??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;");
function iso(x){return x.getFullYear()+"-"+String(x.getMonth()+1).padStart(2,"0")+"-"+String(x.getDate()).padStart(2,"0")}
function startMonday(x){const d=new Date(x);d.setHours(12,0,0,0);const wd=(d.getDay()+6)%7;d.setDate(d.getDate()-wd);return d}
function addDays(x,n){const d=new Date(x);d.setDate(d.getDate()+n);return d}
function setDayHeading(date){const dd=new Date(date+"T12:00:00");headingEl.textContent=dd.toLocaleDateString("tr-TR",{weekday:"long",day:"numeric",month:"long"})+" maçları";}
function renderDays(){
  const names=["Pzt","Sal","Çar","Per","Cum","Cmt","Paz"],arr=[];
  for(let i=0;i<7;i++){const d=addDays(weekStart,i),id=iso(d);arr.push('<button class="day '+(id===selectedDate?'active':'')+'" data-date="'+id+'"><b>'+names[i]+'</b><span>'+d.getDate()+'</span></button>')}
  daysEl.innerHTML=arr.join('');
  daysEl.querySelectorAll('button').forEach(b=>b.onclick=()=>{const d=b.dataset.date;if(!d)return;selectedDate=d;renderDays();setDayHeading(d);loadDay(d)});
  const end=addDays(weekStart,6);weekTitleEl.textContent=weekStart.toLocaleDateString('tr-TR',{day:'numeric',month:'short'})+' – '+end.toLocaleDateString('tr-TR',{day:'numeric',month:'short',year:'numeric'});
}
function chips(){const a=[{code:"ALL",name:"Tümü",emoji:"🌍"},...meta];leaguesEl.innerHTML=a.map(x=>'<button class="chip '+(selected===x.code?'active':'')+'" data-c="'+x.code+'">'+x.emoji+' '+esc(x.name)+'</button>').join('');leaguesEl.querySelectorAll('button').forEach(b=>b.onclick=()=>{selected=b.dataset.c;chips();renderCurrent()});}
function filtered(){return weekFixtures.filter(f=>(selected==="ALL"||f.leagueCode===selected));}
function renderCurrent(){
  const a=filtered();mcEl.textContent=a.length;fcEl.textContent=a.length+' fikstür';setDayHeading(selectedDate);
  if(!a.length){fixturesEl.innerHTML='<div class="empty">NO DATA</div>';return;}
  const groups={};a.forEach(f=>(groups[f.leagueCode]??=[]).push(f));
  fixturesEl.innerHTML=Object.values(groups).map(g=>'<div class="leagueGroup"><div class="leagueTitle">'+esc(g[0].emoji+' '+g[0].country+' · '+g[0].league)+'</div>'+g.map(f=>'<div class="fixture"><div class="fxhead"><span>'+esc(f.round)+'</span><span>'+new Date(f.date).toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'})+'</span></div><div class="teams"><div class="team">'+(f.home.logo?'<img src="'+f.home.logo+'">':'')+esc(f.home.name)+'</div><div class="time">VS</div><div class="team">'+(f.away.logo?'<img src="'+f.away.logo+'">':'')+esc(f.away.name)+'</div></div><button class="analyze" onclick="analyze('+f.id+',this,\''+selectedDate+'\')">PRO ANALİZ</button><div id="a'+f.id+'"></div></div>').join('')+'</div>').join('');
}
async function loadDay(date){
  const requestedDate=String(date);selectedDate=requestedDate;const seq=++loadSeq;renderDays();setDayHeading(requestedDate);weekFixtures=[];mcEl.textContent='—';fcEl.textContent='';fixturesEl.innerHTML='<div class="loader">Seçili gün fikstürü yükleniyor…</div>';
  try{
    let data;
    if(Object.prototype.hasOwnProperty.call(dayCache,requestedDate)){data=dayCache[requestedDate];}
    else{const ctrl=new AbortController(),tm=setTimeout(()=>ctrl.abort(),15000);let r,j;try{r=await fetch('/api/day?date='+encodeURIComponent(requestedDate)+'&_='+Date.now(),{cache:'no-store',signal:ctrl.signal});j=await r.json();}finally{clearTimeout(tm)}if(!j.ok||String(j.date)!==requestedDate)throw Error('NO DATA');data=j.fixtures||[];dayCache[requestedDate]=data;}
    if(seq!==loadSeq||selectedDate!==requestedDate)return;weekFixtures=data;renderCurrent();
  }catch(e){if(seq!==loadSeq||selectedDate!==requestedDate)return;weekFixtures=[];mcEl.textContent='0';fcEl.textContent='0 fikstür';setDayHeading(requestedDate);fixturesEl.innerHTML='<div class="empty">NO DATA</div>';}
}
...
async function analyze(id,b,date){const t=document.getElementById('a'+id);b.disabled=true;b.textContent='HESAPLANIYOR…';t.innerHTML='<div class="loader">Takım performansı ve lig gücü modelleniyor…</div>';try{const r=await fetch('/api/analyze/'+id+'?date='+encodeURIComponent(date||selectedDate)),j=await r.json();if(!j.ok)throw Error(j.error);analyses++;acEl.textContent=analyses;const m=j.model,s=m.stats||{},st=m.standings||{},fx=j.fixture;t.innerHTML='<div class="analysis">'+(m.noBet?'<div class="nobet">NO BET · Model yeterli avantaj görmüyor.</div>':'')+'<div class="grid"><div class="mini"><span>BEKLENEN GOL</span><strong>'+m.expectedGoals.home+' – '+m.expectedGoals.away+'</strong></div><div class="mini"><span>OLASI SKOR</span><strong>'+m.likelyScore+'</strong></div><div class="mini"><span>VERİ KALİTESİ</span><strong>'+m.dataQuality+'/100</strong></div><div class="mini"><span>BEKLENEN KORNER</span><strong>'+(s.expectedCorners??'—')+'</strong></div></div><div class="teamstate"><div class="state"><b>'+esc(fx.home.name)+'</b><span>Lig: '+(st.home?.pos??'—')+'. sıra · '+(st.home?.pts??'—')+' puan</span><span>Ev sırası: '+(st.home?.homePos??'—')+' · Ev PPM: '+(st.home?.homePPG?.toFixed?.(2)??'—')+'</span><span>Form PPM: '+(s.homeFormPPG??'—')+' · SOT: '+(s.homeSOT??'—')+'</span></div><div class="state"><b>'+esc(fx.away.name)+'</b><span>Lig: '+(st.away?.pos??'—')+'. sıra · '+(st.away?.pts??'—')+' puan</span><span>Dep. sırası: '+(st.away?.awayPos??'—')+' · Dep. PPM: '+(st.away?.awayPPG?.toFixed?.(2)??'—')+'</span><span>Form PPM: '+(s.awayFormPPG??'—')+' · SOT: '+(s.awaySOT??'—')+'</span></div></div><div class="section">EN GÜÇLÜ SEÇİMLER</div>'+marketRows(m.recommendations)+'<div class="section">MODEL DAYANAKLARI</div>'+m.reasons.map(x=>'<div class="reason">• '+esc(x)+'</div>').join('')+(m.sampleWarning?'<div class="warn">'+esc(m.sampleWarning)+'</div>':'')+'<div class="section">TÜM ANA MARKETLER</div>'+allMarketsBlock(m.markets)+'</div>';b.textContent='ANALİZ YENİLE';}catch(e){t.innerHTML='<div class="empty">NO DATA</div>';b.textContent='PRO ANALİZ';}finally{b.disabled=false}}
$('prevWeek').onclick=()=>{weekStart=addDays(weekStart,-7);selectedDate=iso(weekStart);renderDays();loadDay(selectedDate)};$('nextWeek').onclick=()=>{weekStart=addDays(weekStart,7);selectedDate=iso(weekStart);renderDays();loadDay(selectedDate)};chips();renderDays();loadDay(selectedDate);
</script></body></html>`;

app.get("/",(req,res)=>res.status(200).type("html").send(HTML));
app.use((req,res)=>res.status(404).json({ok:false,error:"Not found"}));
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.2.5 Professional running on port ${PORT}`));
