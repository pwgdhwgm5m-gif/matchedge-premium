...
/* =========================================================
   MATCHEDGE PREMIUM V7.2.6 PROFESSIONAL
...
async function footballDataUpcoming(date){
  ...
  try{
    const r=await fetch(FD_FIXTURES_URL,{headers:{"User-Agent":"MatchEdge/7.2.6"}});if(!r.ok)throw new Error(`Football-Data fixtures HTTP ${r.status}`);
...
function fixtureMergeKey(f){
  return [f.leagueCode||"",norm(f.home?.name||""),norm(f.away?.name||"")].join("|");
}
...
async function fixturesForDate(date){
  ...
  try{
    ...
    apiRows=rows.map(mapFixture).filter(Boolean);
  }catch(e){
    // API-Football free plan can return only a partial/limited date window.
    // Never block the secondary fixture source because of that.
  }
  try{
    fdRows=await footballDataUpcoming(date);
  }catch(e){
    // If Football-Data fixture feed is temporarily unavailable, keep API rows.
  }
  return mergeFixtures(apiRows,fdRows);
}
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.2.6.1-professional",providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true,odds:false},features:{weeklyCalendar:true,leaguePerformance:true,corners:"9.5-11.5 collapsible",h2hOptional:true}}));
...
app.get("/api/analyze/:id",async(req,res)=>{try{
  ...
  if(!home||!away)return res.status(422).json({ok:false,error:"Bu takım için analiz üretmeye yetecek geçmiş performans verisi bulunamadı. H2H gerekli değildir; takımın kendi maç geçmişi aranmıştır."});
...
</style></head><body><div class="app"><div class="top"><div class="brand">MatchEdge <span class="gold">Premium</span></div><div class="live">● LIVE DATA</div></div><div class="hero"><h1>Maçları haftalık incele.</h1><div class="weeknav"><button id="prevWeek">‹</button><div class="weektitle" id="weekTitle"></div><button id="nextWeek">›</button></div><div class="days" id="days"></div></div><div class="leagues" id="leagues"></div><div class="summary"><div class="stat"><strong id="mc">—</strong><span>SEÇİLİ GÜN MAÇI</span></div><div class="stat"><strong id="ac">0</strong><span>ANALİZ</span></div><div class="stat"><strong>7 GÜN</strong><span>HAFTALIK TAKVİM</span></div></div><div class="head"><h2 id="dayHeading">Maçlar</h2><span id="fc"></span></div><div id="fixtures"><div class="loader">Haftalık fikstür yükleniyor…</div></div></div><script>
const meta=${leagueMeta};let now=new Date(),weekStart=startMonday(now),selectedDate=iso(now),selected="ALL",weekFixtures=[],analyses=0,dayCache={},loadSeq=0;
...
function renderDays(){const names=["Pzt","Sal","Çar","Per","Cum","Cmt","Paz"],arr=[];for(let i=0;i<7;i++){const d=addDays(weekStart,i),id=iso(d);arr.push('<button class="day '+(id===selectedDate?'active':'')+'" data-date="'+id+'"><b>'+names[i]+'</b><span>'+d.getDate()+'</span></button>')}days.innerHTML=arr.join('');days.querySelectorAll('button').forEach(b=>b.onclick=()=>{selectedDate=b.dataset.date;renderDays();loadDay(selectedDate);});const end=addDays(weekStart,6);weekTitle.textContent=weekStart.toLocaleDateString('tr-TR',{day:'numeric',month:'short'})+' – '+end.toLocaleDateString('tr-TR',{day:'numeric',month:'short',year:'numeric'});}
...
function filtered(){return weekFixtures.filter(f=>f.date.slice(0,10)===selectedDate&&(selected==="ALL"||f.leagueCode===selected));}
function render(){const a=filtered();mc.textContent=a.length;fc.textContent=a.length+' fikstür';const dd=new Date(selectedDate+'T12:00:00');dayHeading.textContent=dd.toLocaleDateString('tr-TR',{weekday:'long',day:'numeric',month:'long'})+' maçları';if(!a.length){fixtures.innerHTML='<div class="empty">Bu gün seçili liglerde maç yok.</div>';return;}const groups={};a.forEach(f=>(groups[f.leagueCode]??=[]).push(f));fixtures.innerHTML=Object.values(groups).map(g=>'<div class="leagueGroup"><div class="leagueTitle">'+esc(g[0].emoji+' '+g[0].country+' · '+g[0].league)+'</div>'+g.map(f=>'<div class="fixture"><div class="fxhead"><span>'+esc(f.round)+'</span><span>'+new Date(f.date).toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'})+'</span></div><div class="teams"><div class="team">'+(f.home.logo?'<img src="'+f.home.logo+'">':'')+esc(f.home.name)+'</div><div class="time">VS</div><div class="team">'+(f.away.logo?'<img src="'+f.away.logo+'">':'')+esc(f.away.name)+'</div></div><button class="analyze" onclick="analyze('+f.id+',this)">PRO ANALİZ</button><div id="a'+f.id+'"></div></div>').join('')+'</div>').join('');}
async function loadDay(date){const seq=++loadSeq;selectedDate=date;renderDays();const dd=new Date(date+'T12:00:00');dayHeading.textContent=dd.toLocaleDateString('tr-TR',{weekday:'long',day:'numeric',month:'long'})+' maçları';mc.textContent='—';fc.textContent='';fixtures.innerHTML='<div class="loader">Seçili gün fikstürü yükleniyor…</div>';try{if(dayCache[date]){if(seq!==loadSeq)return;weekFixtures=dayCache[date];render();return;}const r=await fetch('/api/day?date='+date),j=await r.json();if(!j.ok)throw Error(j.error);if(seq!==loadSeq)return;weekFixtures=j.fixtures||[];dayCache[date]=weekFixtures;render();}catch(e){if(seq!==loadSeq)return;weekFixtures=[];mc.textContent='—';fc.textContent='';fixtures.innerHTML='<div class="error">Fikstür alınamadı: '+esc(e.message)+'<br><br><button class="analyze" onclick="loadDay(selectedDate)">TEKRAR DENE</button></div>';}}
...
async function analyze(id,b){const t=document.getElementById('a'+id);b.disabled=true;b.textContent='HESAPLANIYOR…';t.innerHTML='<div class="loader">Takım performansı ve lig gücü modelleniyor…</div>';try{const r=await fetch('/api/analyze/'+id+'?date='+selectedDate),j=await r.json();if(!j.ok)throw Error(j.error);analyses++;ac.textContent=analyses;const m=j.model,s=m.stats||{},st=m.standings||{},fx=j.fixture;t.innerHTML='<div class="analysis">'+(m.noBet?'<div class="nobet">NO BET · Model yeterli avantaj görmüyor.</div>':'')+'<div class="grid"><div class="mini"><span>BEKLENEN GOL</span><strong>'+m.expectedGoals.home+' – '+m.expectedGoals.away+'</strong></div><div class="mini"><span>OLASI SKOR</span><strong>'+m.likelyScore+'</strong></div><div class="mini"><span>VERİ KALİTESİ</span><strong>'+m.dataQuality+'/100</strong></div><div class="mini"><span>BEKLENEN KORNER</span><strong>'+(s.expectedCorners??'—')+'</strong></div></div><div class="teamstate"><div class="state"><b>'+esc(fx.home.name)+'</b><span>Lig: '+(st.home?.pos??'—')+'. sıra · '+(st.home?.pts??'—')+' puan</span><span>Ev sırası: '+(st.home?.homePos??'—')+' · Ev PPM: '+(st.home?.homePPG?.toFixed?.(2)??'—')+'</span><span>Form PPM: '+(s.homeFormPPG??'—')+' · SOT: '+(s.homeSOT??'—')+'</span></div><div class="state"><b>'+esc(fx.away.name)+'</b><span>Lig: '+(st.away?.pos??'—')+'. sıra · '+(st.away?.pts??'—')+' puan</span><span>Dep. sırası: '+(st.away?.awayPos??'—')+' · Dep. PPM: '+(st.away?.awayPPG?.toFixed?.(2)??'—')+'</span><span>Form PPM: '+(s.awayFormPPG??'—')+' · SOT: '+(s.awaySOT??'—')+'</span></div></div><div class="section">EN GÜÇLÜ SEÇİMLER</div>'+marketRows(m.recommendations)+'<div class="section">MODEL DAYANAKLARI</div>'+m.reasons.map(x=>'<div class="reason">• '+esc(x)+'</div>').join('')+(m.sampleWarning?'<div class="warn">'+esc(m.sampleWarning)+'</div>':'')+'<div class="section">TÜM ANA MARKETLER</div>'+allMarketsBlock(m.markets)+'</div>';b.textContent='ANALİZ YENİLE';}catch(e){t.innerHTML='<div class="error">'+esc(e.message)+'</div>';b.textContent='TEKRAR DENE';}finally{b.disabled=false}}
...
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.2 Professional running on port ${PORT}`));
