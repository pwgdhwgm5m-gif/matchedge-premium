...
function buildModel(h,home,away){
  ...
  const cornerMarkets=[];if(cornerLambda!=null){for(const line of [7.5,8.5,9.5,10.5,11.5]){const pp=overPoisson(cornerLambda,line),ep=empiricalOver(teamCornerTotals,line),blend=ep==null?pp:pp*.65+ep*.35;cornerMarkets.push(mk(`Korner ${line} Üst`,blend,"Korner","corners"));cornerMarkets.push(mk(`Korner ${line} Alt`,1-blend,"Korner","corners"));}}
...
function marketRows(xs){return (xs||[]).map(x=>'<div class="market"><b>'+esc(x.name)+'</b><span class="prob">'+x.probability+'%</span><span class="conf">Güven '+x.confidence+'</span></div>').join('');}
