...
async function footballDataUpcoming(date){
  ...
  try{
    const r=await fetch(FD_FIXTURES_URL,{headers:{"User-Agent":"MatchEdge/7.2.2"}});if(!r.ok)throw new Error(`Football-Data fixtures HTTP ${r.status}`);
...
async function fixturesForDate(date){
  ...
  try{
    ...
    if(out.length||date<=today)return out;
  }catch(e){
    if(date<=today)throw e;
  }
  return await footballDataUpcoming(date);
}
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.2.2-professional",providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true,odds:false},features:{weeklyCalendar:true,leaguePerformance:true,corners:"9.5-11.5 collapsible",h2hOptional:true}}));
