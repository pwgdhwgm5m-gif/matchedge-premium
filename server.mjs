...
/* =========================================================
   MATCHEDGE PREMIUM V7.2 PROFESSIONAL
...
async function fixturesForDate(date){const rows=await apiFootball(`/fixtures?date=${encodeURIComponent(date)}`);const out=rows.map(mapFixture).filter(Boolean).sort((a,b)=>a.timestamp-b.timestamp);out.forEach(f=>setCache(`fixture:${f.id}`,f,3600000));return out;}
async function fixturesForRange(from,to){const k=`range:${from}:${to}`,c=getCache(k);if(c)return c;const rows=await apiFootball(`/fixtures?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,900000);const out=rows.map(mapFixture).filter(Boolean).sort((a,b)=>a.timestamp-b.timestamp);out.forEach(f=>setCache(`fixture:${f.id}`,f,3600000));setCache(k,out,900000);return out;}
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.2.0-professional",providers:{fixtures:!!API_KEY,footballData:true,odds:false},features:{weeklyCalendar:true,leaguePerformance:true,corners:"9.5-11.5 collapsible",h2hOptional:true}}));
...
const meta=${leagueMeta};let now=new Date(),weekStart=startMonday(now),selectedDate=iso(now),selected="ALL",weekFixtures=[],analyses=0;
...
async function loadWeek(){renderDays();fixtures.innerHTML='<div class="loader">7 günlük fikstür yükleniyor…</div>';try{const r=await fetch('/api/week?from='+iso(weekStart)),j=await r.json();if(!j.ok)throw Error(j.error);weekFixtures=j.fixtures||[];render();}catch(e){fixtures.innerHTML='<div class="error">'+esc(e.message)+'</div>';}}
...
prevWeek.onclick=()=>{weekStart=addDays(weekStart,-7);selectedDate=iso(weekStart);loadWeek()};nextWeek.onclick=()=>{weekStart=addDays(weekStart,7);selectedDate=iso(weekStart);loadWeek()};chips();loadWeek();
</script></body></html>`;
