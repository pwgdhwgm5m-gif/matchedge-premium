...
async function footballDataUpcoming(date){
  ...
  try{
    const r=await fetch(FD_FIXTURES_URL,{headers:{"User-Agent":"MatchEdge/7.2.4"}});if(!r.ok)throw new Error(`Football-Data fixtures HTTP ${r.status}`);
...
async function fixturesForDate(date){
  ...
  try{
    const seasonFd=await footballDataSeasonUpcoming(date);
    if(seasonFd.length)return seasonFd;
  }catch(e){ /* silent fallback */ }
  return [];
}
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.2.4-professional",providers:{apiFootball:!!API_KEY,footballData:true,fixtureFallback:true,odds:false},features:{weeklyCalendar:true,leaguePerformance:true,corners:"9.5-11.5 collapsible",h2hOptional:true}}));
...
</style></head><body><div class="app"><div class="top"><div class="brand">MatchEdge <span class="gold">Premium</span></div><div class="live">● LIVE DATA · 7.2.4</div></div><div class="hero"><h1>Maçları haftalık incele.</h1><div class="weeknav"><button id="prevWeek">‹</button><div class="weektitle" id="weekTitle"></div><button id="nextWeek">›</button></div><div class="days" id="days"></div></div><div class="leagues" id="leagues"></div><div class="summary"><div class="stat"><strong id="mc">—</strong><span>SEÇİLİ GÜN MAÇI</span></div><div class="stat"><strong id="ac">0</strong><span>ANALİZ</span></div><div class="stat"><strong>7 GÜN</strong><span>HAFTALIK TAKVİM</span></div></div><div class="head"><h2 id="dayHeading">Maçlar</h2><span id="fc"></span></div><div id="fixtures"><div class="loader">Haftalık fikstür yükleniyor…</div></div></div><script>
...
async function loadDay(date){
  ...
  try{
    ...
    else{const r=await fetch('/api/day?date='+encodeURIComponent(requestedDate)+'&_='+Date.now(),{cache:'no-store'}),j=await r.json();if(!j.ok||String(j.date)!==requestedDate)throw Error('NO DATA');data=j.fixtures||[];dayCache[requestedDate]=data;}
...
$('prevWeek').onclick=()=>{weekStart=addDays(weekStart,-7);selectedDate=iso(weekStart);loadDay(selectedDate)};$('nextWeek').onclick=()=>{weekStart=addDays(weekStart,7);selectedDate=iso(weekStart);loadDay(selectedDate)};chips();loadDay(selectedDate);
</script></body></html>`;

app.get("/",(req,res)=>res.status(200).type("html").send(HTML));
app.use((req,res)=>res.status(404).json({ok:false,error:"Not found"}));
app.listen(PORT,"0.0.0.0",()=>console.log(`MatchEdge Premium V7.2.4 Professional running on port ${PORT}`));
