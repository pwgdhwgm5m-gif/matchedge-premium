...
/* =========================================================
   MATCHEDGE PREMIUM V7.2.1 PROFESSIONAL
...
async function fixturesForDate(date){
  ...
  const ttl=date>today?21600000:900000;
...
/* ---------------- Routes ---------------- */
app.get("/api/health",(req,res)=>res.json({ok:true,version:"7.2.1-professional",providers:{fixtures:!!API_KEY,footballData:true,odds:false},features:{weeklyCalendar:true,leaguePerformance:true,corners:"9.5-11.5 collapsible",h2hOptional:true}}));
...
</style></head><body><div class="app"><div class="top"><div class="brand">MatchEdge <span class="gold">Premium</span></div><div class="live">● LIVE DATA</div></div><div class="hero"><small>PROFESSIONAL MATCH INTELLIGENCE</small><h1>Maçları haftalık incele.</h1><p>Lig performansı · ev/deplasman gücü · güncel form · Elo · şut · yarılar · korner · ortak rakipler. H2H varsa yalnız destek sinyalidir.</p><div class="weeknav"><button id="prevWeek">‹</button><div class="weektitle" id="weekTitle"></div><button id="nextWeek">›</button></div><div class="days" id="days"></div></div><div class="leagues" id="leagues"></div><div class="summary"><div class="stat"><strong id="mc">—</strong><span>SEÇİLİ GÜN MAÇI</span></div><div class="stat"><strong id="ac">0</strong><span>ANALİZ</span></div><div class="stat"><strong>7 GÜN</strong><span>HAFTALIK TAKVİM</span></div></div><div class="head"><h2 id="dayHeading">Maçlar</h2><span id="fc"></span></div><div id="fixtures"><div class="loader">Haftalık fikstür yükleniyor…</div></div></div><script>
const meta=${leagueMeta};let now=new Date(),weekStart=startMonday(now),selectedDate=iso(now),selected="ALL",weekFixtures=[],analyses=0,dayCache={};
...
async function loadDay(date){renderDays();fixtures.innerHTML='<div class="loader">Seçili gün fikstürü yükleniyor…</div>';try{if(dayCache[date]){weekFixtures=dayCache[date];render();return;}const r=await fetch('/api/day?date='+date),j=await r.json();if(!j.ok)throw Error(j.error);weekFixtures=j.fixtures||[];dayCache[date]=weekFixtures;render();}catch(e){weekFixtures=[];mc.textContent='—';fc.textContent='';fixtures.innerHTML='<div class="error">Fikstür alınamadı: '+esc(e.message)+'<br><br><button class="analyze" onclick="loadDay(selectedDate)">TEKRAR DENE</button></div>';}}
